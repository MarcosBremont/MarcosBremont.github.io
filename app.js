...
function guardarEdicionActividad(idx) {
  ...
  renderizarActividades(planificacion.actividades);
  mostrarToast(`Actividad "${act.enunciado?.substring(0, 35) || 'sin título'}" actualizada ✓`, 'success');
}
...
function abrirModalInstrumento(idxActividad) {
  ...
  const raHTML = (ra.descripcion || dg.moduloFormativo) ? `
    <div style="background:#E8F5E9;border-left:4px solid #2E7D32;border-radius:8px;padding:12px 16px;margin-bottom:12px;font-size:0.83rem;">
      <div style="font-weight:700;color:#1B5E20;margin-bottom:4px;display:flex;align-items:center;gap:6px;">
        ...
        ${dg.moduloFormativo ? `<span style="font-weight:400;color:#388E3C;margin-left:4px;">— ${dg.moduloFormativo}${dg.codigoModulo ? ' ('+dg.codigoModulo+')' : ''}</span>` : ''}
      </div>
      ${ra.descripcion ? `<div style="color:#2E7D32;line-height:1.5;font-weight:600;">${ra.descripcion}</div>` : ''}
      ${ra.nivelBloom ? `<div style="margin-top:6px;"><span style="background:#2E7D32;color:#fff;border-radius:12px;padding:2px 10px;font-size:0.72rem;font-weight:700;">${ra.nivelBloom.charAt(0).toUpperCase()+ra.nivelBloom.slice(1)}</span></div>` : ''}
    </div>` : '';

  // ── Bloque Planificación Diaria ──────────────────────────────────
  const diariaHTML = (act.fechaStr || act.enunciado) ? `
    <div style="background:#F3E5F5;border-left:4px solid #7B1FA2;border-radius:8px;padding:12px 16px;margin-bottom:12px;font-size:0.83rem;">
      <div style="font-weight:700;color:#4A148C;margin-bottom:4px;display:flex;align-items:center;gap:6px;">
        <span class="material-icons" style="font-size:16px;">today</span>
        Planificación Diaria
        ${act.fechaStr ? `<span style="font-weight:400;color:#6A1B9A;margin-left:4px;">— ${act.fechaStr}</span>` : ''}
      </div>
      ${act.enunciado ? `<div style="color:#4A148C;line-height:1.5;">${act.enunciado}</div>` : ''}
      ${act.horas ? `<div style="margin-top:6px;"><span style="background:#7B1FA2;color:#fff;border-radius:12px;padding:2px 10px;font-size:0.72rem;font-weight:700;">${act.horas}h</span></div>` : ''}
    </div>` : '';
...
let _auditLastMsg  = '';
let _auditLastTime = 0;

function registrarCambio(mensaje) {
  try {
    // Deduplicar: mismo mensaje dentro de 600 ms → ignorar
    const ahora = Date.now();
    if (mensaje === _auditLastMsg && ahora - _auditLastTime < 600) return;
    _auditLastMsg  = mensaje;
    _auditLastTime = ahora;
    ...
    if (window._syncFirebase) _syncFirebase('bitacora', JSON.stringify(log));
...
// ════════════════════════════════════════════════════════════════════
// AUDITORÍA — Panel de actividad
// ════════════════════════════════════════════════════════════════════
function abrirAuditoria() {
  _mostrarPanel('panel-auditoria');
  // Defaults: hoy como hasta, hace 30 días como desde
  const hoy = new Date().toISOString().slice(0, 10);
  const hace30 = new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString().slice(0, 10);
  const desde = document.getElementById('audit-fecha-desde');
  const hasta = document.getElementById('audit-fecha-hasta');
...
function renderizarAuditoria() {
  ...
  let filtrado = log.filter(e => {
    const fecha = e.fecha.slice(0, 10);
    if (desde && fecha < desde) return false;
    if (hasta && fecha > hasta) return false;
    if (buscar && !e.accion.toLowerCase().includes(buscar)) return false;
    return true;
  ...
  if (statsEl) {
    const bloqueos = filtrado.filter(e => e.accion.includes('bloqueada') || e.accion.includes('desbloqueada')).length;
    const logins   = filtrado.filter(e => e.accion.includes('Sesión iniciada') || e.accion.includes('Sesión cerrada')).length;
    const total    = filtrado.length;
    statsEl.innerHTML = [
      ['history', total + ' eventos', '#1565C0', '#E3F2FD'],
      ['lock', bloqueos + ' bloqueos', '#6A1B9A', '#F3E5F5'],
      ['login', logins + ' sesiones', '#1B5E20', '#E8F5E9'],
    ].map(([icon, label, color, bg]) =>
      `<span style="display:inline-flex;align-items:center;gap:5px;background:${bg};color:${color};
        border-radius:20px;padding:4px 12px;font-size:0.78rem;font-weight:700;">
        <span class="material-icons" style="font-size:14px;">${icon}</span>${label}</span>`
    ).join('');
  }
  ...
  if (!lista) return;
  if (!filtrado.length) {
    lista.innerHTML = '<div style="text-align:center;padding:40px;color:#9E9E9E;font-size:0.85rem;">No hay actividad en este periodo</div>';
    return;
  }
  ...
  lista.innerHTML = Object.entries(grupos).map(([dia, entries]) => {
    ...
    const rows = entries.map(e => {
      ...
      const { icon, color } = _auditIcono(e.accion);
      return `<div style="display:flex;align-items:flex-start;gap:10px;padding:8px 12px;border-bottom:1px solid #F5F5F5;">
        ...
        <div style="flex:1;min-width:0;">
          <div class="audit-entry-text" style="font-size:0.83rem;line-height:1.4;">${e.accion}</div>
          <div style="font-size:0.72rem;color:#9E9E9E;margin-top:2px;">${hora}</div>
        </div>
      </div>`;
    }).join('');
...
function limpiarAuditoria() {
  if (!confirm('¿Eliminar todo el historial de actividad? Esta acción no se puede deshacer.')) return;
  localStorage.setItem(BITACORA_KEY, '[]');
  if (window._syncFirebase) _syncFirebase('bitacora', '[]');
  renderizarAuditoria();
...
function _iniciarVigilanciaInactividad() {
  ['mousemove', 'mousedown', 'keydown', 'scroll', 'touchstart', 'click'].forEach(ev => {
    document.addEventListener(ev, _resetActividad, { passive: true });
  });
  _inactivityInterval = setInterval(() => {
    if (!window.currentUser || _pantallaBloqueada) return;
    if (Date.now() - _lastActivity > INACTIVITY_MS) _bloquearPantalla();
  }, 15000); // verificar cada 15 s
}

function _resetActividad() {
  const now = Date.now();
  _lastActivity = now;
  // Persistir en sessionStorage con throttle de 5 s para no saturar escrituras
  if (now - _lastActivityWriteTime > 5000) {
    sessionStorage.setItem(_LAST_ACT_KEY, String(now));
    _lastActivityWriteTime = now;
  }
}

// Determina si al recargar la página la sesión debe estar bloqueada
function _debeBloquearsePorRefresco() {
  // Método 1: flag explícito guardado al bloquear
  if (sessionStorage.getItem(_LOCK_KEY) === '1') return true;
  // Método 2: el tiempo desde la última actividad registrada supera el umbral
  const lastAct = parseInt(sessionStorage.getItem(_LAST_ACT_KEY) || '0');
  return lastAct > 0 && (Date.now() - lastAct) > INACTIVITY_MS;
}

function _bloquearPantalla() {
  if (_pantallaBloqueada || !window.currentUser) return;
  _pantallaBloqueada = true;
  ...
  if (lockTime) {
    ...
    lockTime.textContent = 'Bloqueado a las ' + ahora + ' por inactividad';
  }
...
async function desbloquearPantalla() {
  ...
  const err   = document.getElementById('lock-error');
  const btn   = document.getElementById('lock-btn-unlock');
  if (!pass) { if (err) err.textContent = 'Ingresa tu contraseña.'; return; }

  if (btn) btn.disabled = true;
  try {
    const credential = firebase.auth.EmailAuthProvider.credential(window.currentUser.email, pass);
...
async function _desbloquearConGoogle() {
  const err = document.getElementById('lock-error');
  try {
    const provider = new firebase.auth.GoogleAuthProvider();
    await window.currentUser.reauthenticateWithPopup(provider);
    _completarDesbloqueo();
  } catch (e) {
...
function _completarDesbloqueo() {
  const now = Date.now();
  _pantallaBloqueada = false;
  _lastActivity = now;
  sessionStorage.removeItem(_LOCK_KEY);
  // Reiniciar el timestamp de actividad para que no vuelva a bloquear inmediatamente
  sessionStorage.setItem(_LAST_ACT_KEY, String(now));
  _lastActivityWriteTime = now;
...
function limpiarNotasDocente() {
  ...
  localStorage.setItem(NOTAS_DOCENTE_KEY, '');
  if (window._syncFirebase) _syncFirebase('notas', '');
  const ta = document.getElementById('notas-docente-textarea');
  if (ta) ta.value = '';
...
let _libretaDiaSeleccionado = null;

function cargarLibreta() {
  try { return JSON.parse(localStorage.getItem(LIBRETA_KEY) || '{"entries":[]}'); } catch { return { entries: [] }; }
}

function _guardarLibreta(data) {
  localStorage.setItem(LIBRETA_KEY, JSON.stringify(data));
  if (window._syncFirebase) _syncFirebase('libreta', data);
}

function abrirLibreta() {
  _mostrarPanel('panel-libreta');
  _libretaDiaSeleccionado = new Date().toISOString().split('T')[0];
  renderizarLibreta();
}
...
function renderizarLibreta() {
  ...
  _renderizarEntradasDia(_libretaDiaSeleccionado);
}

function _libretaFechaLabel(fechaISO) {
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
  const ayer = new Date(hoy); ayer.setDate(hoy.getDate() - 1);
  const d = new Date(fechaISO + 'T12:00:00'); d.setHours(0, 0, 0, 0);
  if (d.getTime() === hoy.getTime()) return 'Hoy';
  if (d.getTime() === ayer.getTime()) return 'Ayer';
  return d.toLocaleDateString('es-DO', { weekday: 'short', day: '2-digit', month: 'short' });
}

function _seleccionarDiaLibreta(fecha) {
  _libretaDiaSeleccionado = fecha;
  renderizarLibreta();
}

function _renderizarListaDiasLibreta() {
  const cont = document.getElementById('libreta-lista-dias');
  if (!cont) return;
  const data = cargarLibreta();
  const diasMap = {};
  data.entries.forEach(e => {
    if (!diasMap[e.fecha]) diasMap[e.fecha] = [];
    diasMap[e.fecha].push(e);
  });
  const hoy = new Date().toISOString().split('T')[0];
  const fechas = Object.keys(diasMap).sort((a, b) => b.localeCompare(a));
  if (!diasMap[hoy]) fechas.unshift(hoy);

  cont.innerHTML = fechas.map(fecha => {
    const entries = diasMap[fecha] || [];
    const activo = fecha === _libretaDiaSeleccionado;
    const preview = entries[0]?.titulo || (entries[0]?.texto || '').substring(0, 40) || '—';
    return `<div onclick="_seleccionarDiaLibreta('${fecha}')" style="
        padding:12px 14px;cursor:pointer;border-bottom:1px solid #E0E0E0;
        background:${activo ? '#E0F2F1' : 'transparent'};
        border-left:3px solid ${activo ? '#00695C' : 'transparent'};transition:background 0.15s;">
      <div style="font-size:0.75rem;font-weight:700;color:${activo ? '#00695C' : '#546E7A'};">${_libretaFechaLabel(fecha)}</div>
      <div style="font-size:0.72rem;color:#78909C;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHTML(preview)}</div>
      ${entries.length ? `<div style="font-size:0.68rem;color:#90A4AE;margin-top:2px;">${entries.length} nota${entries.length > 1 ? 's' : ''}</div>` : ''}
    </div>`;
  }).join('');
}
...
function _renderizarEntradasDia(fecha) {
  ...
  if (!cont) return;
  const data = cargarLibreta();
  const entries = data.entries.filter(e => e.fecha === fecha).sort((a, b) => (a.hora || '').localeCompare(b.hora || ''));
  const fechaLabel = new Date(fecha + 'T12:00:00').toLocaleDateString('es-DO', { day: '2-digit', month: 'long', year: 'numeric' });

  cont.innerHTML = `
    <div style="margin-bottom:16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
      <span style="font-size:1rem;font-weight:700;color:#00695C;">${_libretaFechaLabel(fecha)}</span>
      <span style="font-size:0.78rem;color:#90A4AE;">${fechaLabel}</span>
    </div>
    ${entries.length === 0 ? `
      <div style="text-align:center;padding:40px 20px;color:#9E9E9E;">
        <span class="material-icons" style="font-size:2.5rem;opacity:0.3;display:block;">edit_note</span>
        ...
        <div style="background:#fff;border:1.5px solid #B2DFDB;border-radius:10px;padding:14px 16px;margin-bottom:12px;">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
            ...
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0;">
              <span style="font-size:0.68rem;color:#90A4AE;">${e.hora || ''}</span>
              <div style="display:flex;gap:2px;">
                <button onclick="_abrirFormLibreta('${e.id}')" title="Editar"
                  style="background:none;border:none;cursor:pointer;color:#0277BD;padding:4px;border-radius:4px;">
                  <span class="material-icons" style="font-size:16px;">edit</span>
                </button>
                <button onclick="_eliminarEntradaLibreta('${e.id}')" title="Eliminar"
                  style="background:none;border:none;cursor:pointer;color:#C62828;padding:4px;border-radius:4px;">
...
function _abrirFormLibreta(entryId) {
  const data = cargarLibreta();
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="display:flex;flex-direction:column;gap:14px;">
      ...
      <div style="display:flex;gap:8px;justify-content:flex-end;padding-top:4px;">
        <button class="btn-secundario" onclick="cerrarModalBtn()">Cancelar</button>
        <button class="btn-siguiente" onclick="_guardarEntradaLibreta('${entryId || ''}')">
          <span class="material-icons">save</span> Guardar
        </button>
      </div>
    </div>`;
  ...
  document.getElementById('modal-overlay').classList.remove('hidden');
  document.body.style.overflow = 'hidden';
  setTimeout(() => document.getElementById('lib-titulo')?.focus(), 100);
}
...
function _guardarEntradaLibreta(entryId) {
  ...
  cerrarModalBtn();
  renderizarLibreta();
  mostrarToast(entryId ? 'Anotación actualizada' : 'Anotación guardada', 'success');
}

function _eliminarEntradaLibreta(entryId) {
...
function abrirRendimiento() {
  _mostrarPanel('panel-rendimiento');
  _rendCargarSelector();
}

function cerrarRendimiento() { abrirDashboard(); }

function _rendCambiarCurso() {
  const id = document.getElementById('rend-selector-curso')?.value;
  _renderizarGraficasRendimiento(id || null);
}

function _rendCargarSelector() {
  const sel = document.getElementById('rend-selector-curso');
  if (!sel) return;
  const cursos = Object.values(calState.cursos || {});
  if (!cursos.length) {
    sel.innerHTML = '<option value="">Sin cursos registrados</option>';
    _renderizarGraficasRendimiento(null);
    return;
  }
  sel.innerHTML = cursos.map(c => `<option value="${c.id}">${escapeHTML(c.nombre)}</option>`).join('');
  sel.value = cursos[0].id;
  _renderizarGraficasRendimiento(cursos[0].id);
}

function _rendColorPct(pct) {
  if (pct >= 0.7) return { fg: '#2E7D32', bg: 'rgba(46,125,50,0.12)', label: 'Aprobado' };
  if (pct >= 0.6) return { fg: '#F57F17', bg: 'rgba(245,127,23,0.12)', label: 'Regular' };
  return { fg: '#C62828', bg: 'rgba(198,40,40,0.12)', label: 'Reprobado' };
}

function _rendMaxTotal(curso) {
  return Object.values(curso.ras || {}).reduce((s, ra) => s + (ra.valorTotal || 0), 0);
}

function _rendDonutSVG(verde, amarillo, rojo) {
  const total = verde + amarillo + rojo;
  if (!total) return '<div class="rend-empty-small"><span class="material-icons">group_off</span>Sin estudiantes</div>';
  const segs = [
    { val: verde,    color: '#2E7D32', label: 'Aprobados' },
    { val: amarillo, color: '#F57F17', label: 'Regulares' },
    { val: rojo,     color: '#C62828', label: 'Reprobados' },
  ].filter(s => s.val > 0);
  const cx = 80, cy = 80, r = 62, ri = 40;
  let angle = -Math.PI / 2;
  const paths = segs.map(seg => {
    ...
    const d = `M${cx + r*cos1} ${cy + r*sin1} A${r} ${r} 0 ${lg} 1 ${cx + r*cos2} ${cy + r*sin2} L${cx + ri*cos2} ${cy + ri*sin2} A${ri} ${ri} 0 ${lg} 0 ${cx + ri*cos1} ${cy + ri*sin1}Z`;
    angle = end;
    return `<path d="${d}" fill="${seg.color}"/>`;
  });
  const legend = segs.map(s =>
    `<div class="rend-legend-row"><span class="rend-legend-dot" style="background:${s.color}"></span><span>${s.label}: <strong>${s.val}</strong></span></div>`
  ).join('');
  return `<div class="rend-donut-wrap">
    <svg viewBox="0 0 160 160" width="150" height="150" style="overflow:visible">
      ${paths.join('')}
      <text x="${cx}" y="${cy - 6}" text-anchor="middle" font-size="22" font-weight="700" fill="currentColor">${total}</text>
      <text x="${cx}" y="${cy + 13}" text-anchor="middle" font-size="10" fill="#9E9E9E">alumnos</text>
    </svg>
    <div class="rend-legend">${legend}</div>
  </div>`;
}

function _rendBarrasRA(raData) {
  if (!raData.length) return '<div class="rend-empty-small">Sin datos de RA</div>';
  return `<div class="rend-barras-ra">
    ${raData.map(d => {
      const pct = d.max > 0 ? d.avg / d.max : 0;
      ...
      return `<div class="rend-barra-ra-row">
        ...
        <div class="rend-barra-ra-track">
          <div class="rend-barra-ra-fill" style="width:${w}%;background:${col.fg};"></div>
          <span class="rend-barra-ra-pct">${(pct * 100).toFixed(0)}%</span>
        </div>
...
function _renderizarGraficasRendimiento(cursoId) {
  ...
  if (!cursoId) {
    cont.innerHTML = `<div class="rend-empty"><span class="material-icons">bar_chart</span><p>Selecciona un curso para ver sus gráficas.</p></div>`;
    return;
  }

  const curso = calState.cursos[cursoId];
  if (!curso) {
    cont.innerHTML = `<div class="rend-empty"><span class="material-icons">error_outline</span><p>Curso no encontrado.</p></div>`;
    return;
  }

  const estudiantes = curso.estudiantes || [];
  if (!estudiantes.length) {
    cont.innerHTML = `<div class="rend-empty"><span class="material-icons">group</span><p>Este curso no tiene estudiantes registrados.</p></div>`;
    return;
  }
  ...
  const notasFinal = estudiantes.map(est => {
    ...
    const pct = maxTotal > 0 && nota !== null ? nota / maxTotal : 0;
    return { id: est.id, nombre: est.nombre, nota, pct };
  });

  const conNotas = notasFinal.filter(e => e.nota !== null);
  const verdes    = conNotas.filter(e => e.pct >= 0.7);
  const amarillos = conNotas.filter(e => e.pct >= 0.6 && e.pct < 0.7);
  const rojos     = conNotas.filter(e => e.pct < 0.6);
  const sinNotas  = notasFinal.filter(e => e.nota === null);

  const promFinal = _promedioFinal(curso);
  const promPct   = maxTotal > 0 && promFinal !== null ? promFinal / maxTotal : null;
  const colProm   = promPct !== null ? _rendColorPct(promPct) : { fg: '#9E9E9E', bg: 'rgba(158,158,158,0.1)', label: 'Sin datos' };
  ...
  // Promedios por actividad (todas las actividades de todos los RAs)
  const biblio = cargarBiblioteca();
  const actData = [];
  Object.entries(curso.ras || {}).forEach(([rk, ra]) => {
    ...
    (ra.actividades || []).forEach(actId => {
      ...
      const notas = estudiantes
        .map(est => curso.notas?.[est.id]?.[rk]?.[actId])
  ...
  cont.innerHTML = `
    <!-- Fila de KPIs -->
    <div class="rend-kpi-row">
      <div class="rend-kpi" style="border-color:${colProm.fg};background:${colProm.bg};">
        <div class="rend-kpi-num" style="color:${colProm.fg};">${promPct !== null ? (promPct * 100).toFixed(1) + '%' : '—'}</div>
        <div class="rend-kpi-label">Promedio del curso</div>
        <div class="rend-kpi-sub">${promFinal !== null ? promFinal.toFixed(1) + ' / ' + maxTotal + ' pts' : 'Sin calificaciones aún'}</div>
      </div>
      <div class="rend-kpi" style="border-color:#2E7D32;background:rgba(46,125,50,0.08);">
        <div class="rend-kpi-num" style="color:#2E7D32;">${verdes.length}</div>
        <div class="rend-kpi-label" style="color:#2E7D32;">Aprobados ≥ 70%</div>
        <div class="rend-kpi-sub">${conNotas.length ? Math.round(verdes.length/conNotas.length*100) : 0}% del curso</div>
      </div>
      <div class="rend-kpi" style="border-color:#F57F17;background:rgba(245,127,23,0.08);">
        <div class="rend-kpi-num" style="color:#F57F17;">${amarillos.length}</div>
        <div class="rend-kpi-label" style="color:#F57F17;">En riesgo 60–69%</div>
        <div class="rend-kpi-sub">${conNotas.length ? Math.round(amarillos.length/conNotas.length*100) : 0}% del curso</div>
      </div>
      <div class="rend-kpi" style="border-color:#C62828;background:rgba(198,40,40,0.08);">
        <div class="rend-kpi-num" style="color:#C62828;">${rojos.length}</div>
        <div class="rend-kpi-label" style="color:#C62828;">Reprobados < 60%</div>
        <div class="rend-kpi-sub">${conNotas.length ? Math.round(rojos.length/conNotas.length*100) : 0}% del curso</div>
      </div>
      ${sinNotas.length ? `<div class="rend-kpi" style="border-color:#9E9E9E;background:rgba(158,158,158,0.08);">
        <div class="rend-kpi-num" style="color:#9E9E9E;">${sinNotas.length}</div>
        <div class="rend-kpi-label" style="color:#9E9E9E;">Sin calificaciones</div>
        <div class="rend-kpi-sub">Pendientes de notas</div>
      </div>` : ''}
    </div>
    ...
    <div class="rend-charts-row">
      <div class="rend-card">
        <div class="rend-card-title"><span class="material-icons">donut_large</span>Distribución del curso</div>
        ${_rendDonutSVG(verdes.length, amarillos.length, rojos.length)}
      </div>
      <div class="rend-card rend-card-grow">
        <div class="rend-card-title"><span class="material-icons">bar_chart</span>Promedio por Resultado de Aprendizaje</div>
        ${_rendBarrasRA(raData)}
      </div>
    </div>

    <!-- Barras de actividades -->
    ${actData.length ? `<div class="rend-card">
      <div class="rend-card-title"><span class="material-icons">assignment</span>Promedio por Actividad de Evaluación</div>
      ${_rendBarrasActividades(actData)}
    </div>` : ''}

    <!-- Estudiantes en riesgo -->
    ${(() => {
      const enRiesgo = _estudiantesEnRiesgo(curso);
      if (!enRiesgo.length) return `<div class="rend-card rend-sin-riesgo">
        <span class="material-icons" style="color:#2E7D32;font-size:20px;">check_circle</span>
        <span>Sin estudiantes en riesgo con los umbrales actuales</span>
      </div>`;
      return `<div class="rend-card rend-card-riesgo">
        <div class="rend-card-title rend-title-riesgo">
          <span class="material-icons">person_alert</span>
          Estudiantes en riesgo
          <span class="rend-riesgo-badge">${enRiesgo.length}</span>
          <span class="rend-riesgo-sub">Umbral: ${localStorage.getItem('cfg_umbral_riesgo')||'70'}% · ${localStorage.getItem('cfg_umbral_acts')||'3'}+ acts sin nota</span>
        </div>
        <div class="rend-riesgo-lista">
          ${enRiesgo.map(e => {
            const col = _rendColorPct(e.pct ?? 0);
            const w = e.pct !== null ? Math.min(e.pct * 100, 100).toFixed(1) : 0;
            return `<div class="rend-riesgo-row">
              <div class="rend-riesgo-info">
                <span class="rend-riesgo-nombre">${escapeHTML(e.nombre)}</span>
                <div class="rend-riesgo-tags">
                  ${e.razones.map(r => `<span class="rend-riesgo-tag">${escapeHTML(r)}</span>`).join('')}
                </div>
              </div>
              <div class="rend-riesgo-barra-wrap">
                <div class="rend-rank-barra-fill" style="width:${w}%;background:${col.fg};height:100%;border-radius:4px;"></div>
              </div>
              <span class="rend-riesgo-pct" style="color:${col.fg};">${e.pct !== null ? (e.pct*100).toFixed(1)+'%' : 'S/N'}</span>
            </div>`;
          }).join('')}
        </div>
      </div>`;
    ...
    <div class="rend-card">
      <div class="rend-card-title"><span class="material-icons">format_list_numbered</span>Ranking de estudiantes</div>
      <div class="rend-ranking">
        ${[...notasFinal].sort((a, b) => (b.nota ?? -1) - (a.nota ?? -1)).map((e, i) => {
          const col = e.nota !== null ? _rendColorPct(e.pct) : { fg: '#9E9E9E', bg: 'rgba(0,0,0,0.04)', label: 'Sin notas' };
          const w = e.nota !== null ? Math.min(e.pct * 100, 100).toFixed(1) : 0;
          return `<div class="rend-rank-row">
            <span class="rend-rank-pos" style="color:${col.fg};">${e.nota !== null ? '#' + (i+1) : '—'}</span>
            <span class="rend-rank-nombre">${escapeHTML(e.nombre)}</span>
            <div class="rend-rank-barra-wrap">
              <div class="rend-rank-barra-fill" style="width:${w}%;background:${col.fg};"></div>
            </div>
            <span class="rend-rank-pct" style="color:${col.fg};">${e.nota !== null ? (e.pct*100).toFixed(1)+'%' : 'S/N'}</span>
            <span class="rend-rank-nota">${e.nota !== null ? e.nota.toFixed(1)+'/'+maxTotal : '—'}</span>
          </div>`;
        }).join('')}
      </div>
    </div>
...
function abrirCalificaciones(cursoId) {
  const preselect = cursoId || calState.cursoActivoId;
...
function _irACalificacionesPlan(cursoId, planId) {
  ...
  const curso = calState.cursos[cursoId];
  if (curso && planId) curso.planActivaId = planId;
  guardarCalificaciones();
  abrirCalificaciones();
}
...
function _generarNotificaciones() {
  ...
  const thoy = tareas.filter(t => t.estado !== 'entregada' && t.fechaVencimiento &&
    new Date(t.fechaVencimiento + 'T12:00:00').setHours(0, 0, 0, 0) === hoy.getTime());
  if (thoy.length) {
    notifs.push({
  ...
  const tman = tareas.filter(t => t.estado !== 'entregada' && t.fechaVencimiento &&
    new Date(t.fechaVencimiento + 'T12:00:00').setHours(0, 0, 0, 0) === manana.getTime());
  if (tman.length) {
    notifs.push({
...
// ════════════════════════════════════════════════════════════════════
// MÓDULO: PARTICIPACIÓN EN CLASE
// ════════════════════════════════════════════════════════════════════
const PARTICIP_KEY = 'planificadorRA_participacion_v1';
...
function toggleVistaParticipacion() {
  const panel = document.getElementById('cal-particip-panel');
  const btn = document.getElementById('btn-toggle-participacion');
  if (!panel) return;
  _participPanelAbierto = !_participPanelAbierto;
  panel.classList.toggle('hidden', !_participPanelAbierto);
  if (btn) {
    btn.style.background = _participPanelAbierto ? '#6A1B9A' : '#F3E5F5';
    btn.style.color = _participPanelAbierto ? '#fff' : '#6A1B9A';
    btn.style.borderColor = _participPanelAbierto ? '#6A1B9A' : '#CE93D8';
  }
...
function renderizarParticipacion() {
  const wrap = document.getElementById('cal-particip-wrap');
  if (!wrap) return;
  const curso = calState.cursos[calState.cursoActivoId];
  if (!curso || !(curso.estudiantes || []).length) {
    wrap.innerHTML = '<div class="asist-empty"><span class="material-icons">group_off</span><p>Sin estudiantes en este curso.</p></div>';
    return;
  }
  const hoy = new Date().toISOString().split('T')[0];
  ...
  curso.estudiantes.forEach(e => { const v = diaData[e.id]; if (v) counts[v] = (counts[v] || 0) + 1; });
  const registrados = Object.keys(diaData).length;

  wrap.innerHTML = `
    <div class="asist-fecha-header">
      <span class="material-icons">today</span>
      <span>${fechaFmt}</span>
      <input type="date" value="${fecha}" onchange="_cambiarFechaParticip(this.value)"
        style="margin-left:auto;padding:5px 10px;border:1.5px solid #CE93D8;border-radius:8px;font-size:0.82rem;">
    </div>
    <div class="asist-summary-bar">
      <div class="asist-sum-item" style="color:#6A1B9A;"><span class="material-icons">star</span>${counts.D} <span>Destacado</span></div>
      <div class="asist-sum-item" style="color:#2E7D32;"><span class="material-icons">check_circle</span>${counts.P} <span>Participó</span></div>
      <div class="asist-sum-item" style="color:#C62828;"><span class="material-icons">cancel</span>${counts.N} <span>No participó</span></div>
      <div class="asist-sum-item" style="color:#546E7A;"><span class="material-icons">group</span>${registrados}/${curso.estudiantes.length} <span>Registrados</span></div>
    </div>
    <div class="asist-acciones-rapidas">
      <button class="asist-btn-todos" onclick="_marcarTodosParticip('P')" style="border-color:#A5D6A7;color:#2E7D32;">
        <span class="material-icons">done_all</span> Todos participaron
      </button>
      ...
      ${curso.estudiantes.map((est, i) => {
        ...
        return `<div class="asist-fila" id="particip-fila-${est.id}">
          ...
          <div class="asist-btns">
            ...
            <button class="particip-btn P ${v === 'P' ? 'activo' : ''}" onclick="marcarParticipacion('${est.id}','P')" title="Participó">
              <span class="material-icons">check_circle</span>Participó
            </button>
            <button class="particip-btn N ${v === 'N' ? 'activo' : ''}" onclick="marcarParticipacion('${est.id}','N')" title="No participó">
              <span class="material-icons">cancel</span>No participó
            </button>
          </div>
        </div>`;
      }).join('')}
...
function _cambiarFechaParticip(fecha) {
  _participFechaSeleccionada = fecha;
  renderizarParticipacion();
}
...
function marcarParticipacion(estudianteId, estado, fechaOverride) {
  ...
  const wrap = document.getElementById('cal-particip-wrap');
  if (wrap) {
    const items = wrap.querySelectorAll('.asist-sum-item');
    if (items[0]) items[0].innerHTML = `<span class="material-icons">star</span>${counts.D} <span>Destacado</span>`;
    if (items[1]) items[1].innerHTML = `<span class="material-icons">check_circle</span>${counts.P} <span>Participó</span>`;
    if (items[2]) items[2].innerHTML = `<span class="material-icons">cancel</span>${counts.N} <span>No participó</span>`;
    if (items[3]) items[3].innerHTML = `<span class="material-icons">group</span>${Object.keys(diaData).length}/${(curso?.estudiantes || []).length} <span>Registrados</span>`;
  }
}

function _marcarTodosParticip(estado) {
  const cursoId = calState.cursoActivoId;
  const curso = calState.cursos[cursoId];
  if (!curso) return;
...
function _estadoTarea(tarea) {
  ...
  if (tarea.estado === 'no_entregada') return 'no_entregada';
  if (!tarea.fechaLimite) return 'pendiente';
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
  const fl = new Date(tarea.fechaLimite + 'T12:00:00'); fl.setHours(0, 0, 0, 0);
...
function _diasRestantes(fechaLimite) {
  ...
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0);
  const fl = new Date(fechaLimite + 'T12:00:00'); fl.setHours(0, 0, 0, 0);
...
function renderizarTareas() {
  ...
  const counts = { todas: tareas.length, pendiente: 0, entregada: 0, vencida: 0, no_entregada: 0 };
  tareas.forEach(t => { const e = _estadoTarea(t); counts[e] = (counts[e] || 0) + 1; });
  ['todas', 'pendiente', 'entregada', 'vencida', 'no_entregada'].forEach(f => {
  ...
  const _renderTarjeta = t => {
    ...
    const estadoCls = { pendiente: 'tarea-pendiente', entregada: 'tarea-entregada', vencida: 'tarea-vencida', no_entregada: 'tarea-no-entregada' }[t._estado];
    const estadoLabel = { pendiente: 'Pendiente', entregada: 'Entregada', vencida: 'Vencida', no_entregada: 'No entregada' }[t._estado];
    let diasLabel = '';
    ...
    return `<div class="tarea-card ${estadoCls}" id="tarea-card-${t.id}">
      <div class="tarea-card-header">
        ...
        <div style="margin-left:auto;display:flex;gap:6px;">
          ${(t._estado === 'pendiente' || t._estado === 'vencida')
        ? `<button class="tarea-btn-accion" onclick="marcarTareaEntregada('${t.id}')" title="Marcar como entregada"
              ...
              <button class="tarea-btn-accion" onclick="marcarTareaNoEntregada('${t.id}')" title="No entregada"
                style="background:#ECEFF1;border-color:#B0BEC5;color:#546E7A;">
                <span class="material-icons" style="font-size:15px;">unpublished</span>
              </button>`
  ...
  container.innerHTML = ordenGrupos.map(grupo => {
    ...
    const nNoEntr = items.filter(t => t._estado === 'no_entregada').length;
    const headerColor = nVenc > 0 ? '#C62828' : nPend > 0 ? '#E65100' : nNoEntr > 0 ? '#546E7A' : '#2E7D32';
    const badges = [
      ...
      nNoEntr ? `<span style="background:#ECEFF1;color:#455A64;border-radius:12px;padding:2px 9px;font-size:0.7rem;font-weight:700;">${nNoEntr} no entregada(s)</span>` : '',
    ].filter(Boolean).join('');
...
async function _generarSesionConIA(actId, act, ec) {
  ...
  try {
    ...
    const toStr = (val) => {
      ...
      if (Array.isArray(val)) return val.map(item => {
        ...
        if (typeof item === 'object' && item) {
          // Convertir objeto a "clave: valor" líneas legibles
          return Object.entries(item).map(([k, v]) => `${k}: ${typeof v === 'string' ? v : String(v)}`).join('\n');
        }
        return String(item);
      }).join('\n\n');
      if (typeof val === 'object') {
        // Convertir objeto a líneas legibles clave: valor
        return Object.entries(val).map(([k, v]) => {
          if (typeof v === 'string') return `${k}: ${v}`;
          if (Array.isArray(v)) return `${k}:\n${toStr(v)}`;
          if (typeof v === 'object' && v) return `${k}: ${Object.entries(v).map(([k2, v2]) => `${k2}: ${v2}`).join(', ')}`;
          return `${k}: ${String(v)}`;
        }).join('\n');
      }
      return String(val);
    ...
    // normalizar: convertir cualquier formato a texto legible en los campos esperados
    if (!d.apertura && !d.procedimental && !d.sintesis) {
      // Caso especial: {detalles: [...]} — expandir el array
      if (d.detalles && Array.isArray(d.detalles)) {
        d = { apertura: toStr(d.detalles) };
      }
      const entries = Object.entries(d);
      if (entries.length > 0) {
        const norm = {};
        // Convertir todo a texto limpio sin JSON
        const textos = entries.map(([k, v]) => {
          const texto = typeof v === 'string' ? v : toStr(v);
          // No prefijar con la clave si es genérica como "detalles" o "respuesta"
          if (['detalles', 'respuesta', 'resultado', 'contenido', 'data'].includes(k.toLowerCase())) return texto;
          return `${k}: ${texto}`;
        });
        const total = textos.length;
        if (total >= 7) {
          // Muchos campos: distribuir entre inicio/desarrollo/cierre
          const corte1 = Math.max(1, Math.ceil(total * 0.3));
          const corte2 = Math.max(corte1 + 1, Math.ceil(total * 0.75));
          norm.apertura = textos.slice(0, corte1).join('\n\n');
          norm.encuadre = textos.slice(corte1, corte1 + 1).join('\n\n');
          norm.organizacion = textos.slice(corte1 + 1, corte1 + 2).join('\n\n');
          norm.procedimental = textos.slice(corte1 + 2, corte2).join('\n\n');
          norm.conceptual = textos.slice(corte2, corte2 + 1).join('\n\n');
          norm.sintesis = textos.slice(corte2 + 1).join('\n\n');
        } else {
          // Pocos campos: todo al campo apertura, el resto usa fallback local
...
const TOUR_PASOS = [
  ...
  },
  {
    ...
    desc: 'Crea y gestiona planes de enseñanza por unidades o bloques. Aquí defines tus RAs, actividades y fechas.',
    icono: 'assignment',
    target: 'button[onclick="abrirPlanificaciones()"]',
  },
  {
    titulo: 'Diarias',
    desc: 'Registra tu diario de clases: qué enseñaste, logros, dificultades y asistencia.',
    icono: 'today',
    target: 'button[onclick="abrirDiarias()"]',
  },
  {
    titulo: 'Calificaciones',
    desc: 'Lleva el control de las notas de tus estudiantes por RA y actividad. Visualiza promedios al instante.',
    icono: 'grade',
    target: 'button[onclick="abrirCalificaciones()"]',
  },
  {
    titulo: 'Horario de Clases',
    desc: 'Consulta tus clases de hoy y la semana, organizadas por grupo. Perfecta para un vistazo rápido.',
    icono: 'schedule',
    target: 'button[onclick="abrirHorario()"]',
  },
  {
    titulo: 'Tareas y Pendientes',
    desc: 'Gestiona tus tareas docentes con fechas límite y prioridades. Nunca pierdas un pendiente importante.',
    icono: 'task_alt',
    target: 'button[onclick="abrirTareas()"]',
  },
  {
    titulo: 'Notas Rápidas',
    desc: 'Captura ideas, observaciones o recordatorios en segundos. Tus notas, siempre a mano.',
    icono: 'sticky_note_2',
    target: 'button[onclick="abrirNotas()"]',
  },
  {
    titulo: 'Mi Libreta',
    desc: 'Una agenda organizada por día para tus anotaciones con título. Historial de entradas por fecha.',
    icono: 'menu_book',
    target: 'button[onclick="abrirLibreta()"]',
  },
  {
    titulo: 'Gráficas de Rendimiento',
    desc: 'Visualiza el rendimiento de tus cursos con gráficas de barras, donuts y ranking de estudiantes.',
    icono: 'insights',
    target: 'button[onclick="abrirRendimiento()"]',
  },
  {
    titulo: '¡Todo listo!',
...
function _tourRenderizarPaso(n) {
  ...
  const btnSig = document.getElementById('tour-btn-sig');
  if (btnAnt) btnAnt.style.display = n === 0 ? 'none' : '';
  if (btnSig) btnSig.textContent = n === total - 1 ? '¡Listo! ✓' : 'Siguiente →';

  const targetEl = paso.target ? document.querySelector(paso.target) : null;
  _tourPosicionarCaja(targetEl);
}

function _tourPosicionarCaja(targetEl) {
  const spotlight = document.getElementById('tour-spotlight');
  const caja = document.getElementById('tour-caja');
  if (!spotlight || !caja) return;

  const _centrar = () => {
    spotlight.style.cssText = 'position:fixed;width:0;height:0;top:50%;left:50%;box-shadow:0 0 0 9999px rgba(0,0,0,0.72);border-radius:0;z-index:9001;pointer-events:none;';
    caja.style.cssText = 'position:fixed;z-index:9002;top:50%;left:50%;transform:translate(-50%,-50%);opacity:1;';
  };
  ...
  // Ocultar tarjeta mientras scrolleamos para que no "vuele" por la pantalla
  caja.style.opacity = '0';

  // Scroll al elemento para que esté visible en pantalla
  targetEl.scrollIntoView({ behavior: 'smooth', block: 'center' });

  // Esperar a que el scroll termine, luego posicionar
  setTimeout(() => {
    ...
    const cajaW = Math.min(360, window.innerWidth - 32);
    const cajaH = 270;
    let top, left;

    if (rect.bottom + pad + cajaH + 16 < window.innerHeight) {
      top = rect.bottom + pad + 8;
    } else {
      top = rect.top - pad - cajaH - 8;
      if (top < 16) top = 16;
    }

    left = rect.left + rect.width / 2 - cajaW / 2;
    left = Math.max(16, Math.min(left, window.innerWidth - cajaW - 16));
...
function _arrancarApp() {
  ...
  if (_debeBloquearsePorRefresco()) {
    _pantallaBloqueada = false; // resetear bandera para que _bloquearPantalla no la ignore
    _bloquearPantalla();
    return;
  }
  // Registrar la actividad inicial para que el timestamp quede guardado desde el primer momento
  sessionStorage.setItem(_LAST_ACT_KEY, String(Date.now()));
  registrarCambio('Sesión iniciada — ' + (window.currentUser?.email || ''));
}
...
function abrirConfiguracion() {
  ...
  if (cfgUmbralActs) cfgUmbralActs.value = localStorage.getItem('cfg_umbral_acts') || '3';
  const cfgTouch = document.getElementById('cfg-touch-mode');
  if (cfgTouch) cfgTouch.checked = tactil;
  // Asistencia activa
  const cfgAsist = document.getElementById('cfg-asistencia-activa');
  if (cfgAsist) cfgAsist.checked = localStorage.getItem('cfg_asistencia_activa') !== 'false';
  const umbralRow = document.getElementById('cfg-asistencia-umbral-row');
  if (umbralRow) umbralRow.style.display = (localStorage.getItem('cfg_asistencia_activa') !== 'false') ? '' : 'none';
...
function _aplicarAsistenciaActiva() {
  // 'true' o null → activo; 'false' → desactivado
  const activa = localStorage.getItem('cfg_asistencia_activa') !== 'false';

  // Sincronizar checkbox en config
  const chk = document.getElementById('cfg-asistencia-activa');
  if (chk) chk.checked = activa;

  // Mostrar/ocultar fila de umbral
  const umbralRow = document.getElementById('cfg-asistencia-umbral-row');
  if (umbralRow) umbralRow.style.display = activa ? '' : 'none';

  // Mostrar/ocultar botón de asistencia en el libro de calificaciones
  const btnToggle = document.getElementById('btn-toggle-asistencia');
  if (btnToggle) btnToggle.style.display = activa ? '' : 'none';

  // Si está desactivada y la vista de asistencia está abierta, ocultarla
  if (!activa) {
    const panelAsist = document.getElementById('panel-asistencia') ||
                       document.querySelector('.asistencia-panel');
    if (panelAsist) panelAsist.classList.add('hidden');
  }
}
...
function _renderizarSaludo() {
  ...
  el.innerHTML = `
    <div class="dash-greeting-left">
      ...
      <div class="dash-greeting-sub">Sistema de Planificación Educativa · República Dominicana <span class="dash-version-badge" onclick="abrirAcercaDe()" title="Ver novedades de la versión">v14.0</span></div>
    </div>
...
function _estudiantesEnRiesgo(curso) {
  const umbralPct  = (parseInt(localStorage.getItem('cfg_umbral_riesgo') || '70')) / 100;
  const umbralActs = parseInt(localStorage.getItem('cfg_umbral_acts')    || '3');
  const maxTotal   = _rendMaxTotal(curso);

  return (curso.estudiantes || []).map(est => {
    const nota = _calcNotaFinal(curso, est.id);
    const pct  = maxTotal > 0 && nota !== null ? nota / maxTotal : null;

    // Contar actividades sin calificar
    let actsSinNota = 0;
    Object.entries(curso.ras || {}).forEach(([rk, ra]) => {
      (ra.actividades || []).forEach(actId => {
        const n = curso.notas?.[est.id]?.[rk]?.[actId];
        if (n === undefined || n === null) actsSinNota++;
      });
    });

    const bajoProm       = pct !== null && pct < umbralPct;
    const muchasActsSin  = actsSinNota >= umbralActs;
    if (!bajoProm && !muchasActsSin) return null;

    return {
      id: est.id, nombre: est.nombre, nota, pct, actsSinNota,
      razones: [
        bajoProm      ? `Promedio ${pct !== null ? (pct*100).toFixed(1)+'%' : '—'} < ${Math.round(umbralPct*100)}%` : null,
        muchasActsSin ? `${actsSinNota} actividad${actsSinNota !== 1 ? 'es' : ''} sin calificar` : null,
      ].filter(Boolean),
    };
  }).filter(Boolean);
}
...
function _renderizarAlertas() {
  ...
  const urgentes = tareas.filter(t => {
    ...
    const fl = new Date(t.fechaLimite + 'T12:00:00'); fl.setHours(0, 0, 0, 0);
    return fl <= manana;
  });
  urgentes.forEach(t => {
    const fl = new Date(t.fechaLimite + 'T12:00:00'); fl.setHours(0, 0, 0, 0);
  ...
  Object.values(calState.cursos || {}).forEach(curso => {
    ...
    const nombres = enRiesgo.slice(0, 3).map(e => e.nombre.split(' ')[0]).join(', ');
    const extra = enRiesgo.length > 3 ? ` y ${enRiesgo.length - 3} más` : '';
    alertas.push({
      tipo: 'riesgo',
      icono: 'person_alert',
      msg: `${enRiesgo.length} estudiante${enRiesgo.length !== 1 ? 's' : ''} en riesgo en "${curso.nombre}": ${nombres}${extra}`,
      accion: `abrirRendimiento()`,
      label: 'Ver gráficas',
    });
  });
...
function _renderizarClasesDia(contId, fechaLabelId, offsetDias) {
    ...
    (reg.planificacion?.actividades || []).forEach(act => {
      ...
      if (fa.getTime() === target.getTime()) {
        ...
        cursosConPlan.forEach(c => {
          ...
          const key = _normStr(c.nombre) + '|||' + modulo;
          if (!planPorSeccion[key]) planPorSeccion[key] = [];
          planPorSeccion[key].push({ act, dg: reg.planificacion?.datosGenerales || {}, ra: reg.planificacion?.ra || {}, planId: reg.id, cursoId: c.id, coursePlanActivaId: c.planActivaId || null });
        });
      }
    });
  });
  ...
  cont.innerHTML = horario.map(e => {
    ...
    let sesiones = planPorSeccion[_normSec + '|||' + _normMat] || [];
    if (!sesiones.length && _normSec) {
      // Fallback: buscar por sección + palabras clave (≥60% de palabras >3 chars coinciden)
      const _words = str => str.split(' ').filter(w => w.length > 3);
      const wa = _words(_normMat);
      const fallbackKey = Object.keys(planPorSeccion).find(k => {
        const parts = k.split('|||');
        if (parts[0] !== _normSec || parts.length < 2) return false;
        const wb = _words(parts[1]);
        const wbSet = new Set(wb);
        const shared = wa.filter(w => wbSet.has(w)).length;
        return Math.min(wa.length, wb.length) > 0 && shared / Math.min(wa.length, wb.length) >= 0.6;
      });
      if (fallbackKey) sesiones = planPorSeccion[fallbackKey] || [];
    }
    // Dedup: si hay actividades de múltiples planes con el mismo módulo, preferir el plan activo del curso
    if (sesiones.length > 1) {
      const planIdsUnicos = [...new Set(sesiones.map(s => s.planId))];
      if (planIdsUnicos.length > 1) {
        // Buscar planes que sean el planActivaId de su curso
        const activosIds = new Set(
          sesiones.filter(s => s.coursePlanActivaId && s.planId === s.coursePlanActivaId).map(s => s.planId)
        );
    ...
    const dataClase = JSON.stringify({
      ...
      sesiones: sesiones.map(s => ({ enunciado: s.act.enunciado || '', actId: s.act.id || '', planId: s.planId || '', cursoId: s.cursoId || '', instrumento: s.act.instrumento || null, dg: s.dg, ra: s.ra || {} })),
...
function abrirModalClase(encodedData) {
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="display:flex;flex-direction:column;gap:14px;">
      ...
      <!-- Ir a calificaciones -->
      <button onclick="cerrarModalBtn();_irACalificacionesPlan('${sesionInfo?.cursoId || cursoConSeccion?.id || ''}','${sesionInfo?.planId || ''}');"
...
function _renderizarResumenCursos() {
  ...
  cont.innerHTML = `<div class="dash-cursos-grid">` + cursos.map(curso => {
    ...
    return `<div class="dash-curso-card" onclick="abrirCalificaciones('${curso.id}')">
...
const BLOG_KEY = 'planificadorRA_blog_v1';
...
function abrirBlog() {
  cargarCalificaciones();
  _mostrarPanel('panel-blog');
  const sel = document.getElementById('blog-filtro-curso');
  if (sel) {
    ...
    sel.innerHTML = '<option value="">Todos los cursos</option>'
      + cursos.map(c => `<option value="${escapeHTML(c.id)}">${escapeHTML(c.nombre)}</option>`).join('');
  }
  renderizarBlog();
}
function cerrarBlog() { abrirDashboard(); }

function renderizarBlog() {
  const cont = document.getElementById('blog-container');
  if (!cont) return;
  ...
  // Link bar — siempre por curso para evitar mezcla de grupos
  const linkBar = document.getElementById('blog-link-bar');
  const linkUrl = document.getElementById('blog-link-url');
  if (linkBar && linkUrl && window.currentUser) {
    const base = window.location.href.replace(/[^/]*$/, '');
    const uid  = window.currentUser.uid;
    if (filtro) {
      linkUrl.textContent = base + 'blog.html?uid=' + uid + '&curso=' + filtro;
      linkBar.classList.remove('hidden');
    } else {
      // Sin filtro: mostrar links de todos los cursos que tengan posts publicados
      const cursosConPosts = [...new Map(
        (blog.posts || []).filter(p => p.publicado).map(p => [p.cursoId, p.cursoNombre])
      ).entries()];
      if (cursosConPosts.length > 0) {
        linkUrl.innerHTML = cursosConPosts.map(([cid, cnombre]) => {
          const url = base + 'blog.html?uid=' + uid + '&curso=' + cid;
          return `<span style="display:block;margin:2px 0;"><strong>${escapeHTML(cnombre)}:</strong> ${escapeHTML(url)}</span>`;
        }).join('');
        linkBar.classList.remove('hidden');
      } else {
...
function _blogRenderCard(p) {
  const tipoMap = {
    pc:       { label: 'Tarea PC',       icon: 'computer',   color: '#1565C0', bg: '#E3F2FD' },
    cuaderno: { label: 'Tarea Cuaderno', icon: 'menu_book',  color: '#2E7D32', bg: '#E8F5E9' },
    info:     { label: 'Información',    icon: 'info',       color: '#E65100', bg: '#FFF3E0' }
  };
  const ti = tipoMap[p.tipo] || tipoMap.info;
  const fecha = p.fechaLimite
    ? new Date(p.fechaLimite + 'T12:00:00').toLocaleDateString('es', { day: '2-digit', month: 'short', year: 'numeric' })
    : null;
  const adjuntos = p.adjuntos || [];
  return `<div class="blog-post-card">
    <div class="blog-post-header">
      <span class="blog-tipo-badge" style="background:${ti.bg};color:${ti.color};">
        <span class="material-icons" style="font-size:13px;">${ti.icon}</span> ${ti.label}
      </span>
      ${p.publicado
        ? '<span class="blog-pub-badge pub">Publicado</span>'
        : '<span class="blog-pub-badge borrador">Borrador</span>'}
    </div>
    <div class="blog-post-title">${escapeHTML(p.titulo)}</div>
    <div class="blog-post-meta">
      <span><span class="material-icons" style="font-size:13px;">school</span> ${escapeHTML(p.cursoNombre || '—')}</span>
      <span><span class="material-icons" style="font-size:13px;">bookmark</span> ${escapeHTML((p.raLabel || '—').substring(0, 45))}</span>
      ${fecha ? `<span><span class="material-icons" style="font-size:13px;">event</span> Límite: ${fecha}</span>` : ''}
      <span><span class="material-icons" style="font-size:13px;">schedule</span> ${new Date(p.creadoEn).toLocaleDateString('es')}</span>
    </div>
    <div class="blog-post-contenido">${escapeHTML(p.contenido || '').replace(/\n/g, '<br>')}</div>
    ${adjuntos.length > 0
      ? `<div class="blog-adjuntos-row">${adjuntos.map(a =>
          `<span class="blog-adjunto-chip"><span class="material-icons" style="font-size:13px;">attach_file</span> ${escapeHTML(a.nombre)}</span>`
        ).join('')}</div>`
      : ''}
    <div class="blog-post-actions">
      <button onclick="abrirModalPost('${p.id}')"><span class="material-icons">edit</span> Editar</button>
      ${p.publicado
        ? `<button onclick="despublicarPost('${p.id}')" style="color:#E65100;"><span class="material-icons">visibility_off</span> Despublicar</button>`
        : `<button onclick="publicarPost('${p.id}')" style="color:#2E7D32;"><span class="material-icons">publish</span> Publicar</button>`}
      ${p.tipo === 'pc'
...
function abrirModalPost(id) {
  cargarCalificaciones();
  const blog = cargarBlog();
  const post = id ? (blog.posts || []).find(p => p.id === id) : null;
  const cursos = Object.values(calState.cursos || {}).sort((a, b) => a.nombre.localeCompare(b.nombre));
  const opsCursos = cursos.map(c =>
    `<option value="${escapeHTML(c.id)}" ${post?.cursoId === c.id ? 'selected' : ''}>${escapeHTML(c.nombre)}</option>`
  ).join('');

  window._blogTempAdjuntos = (post?.adjuntos || []).map(a => ({ ...a }));

  document.getElementById('modal-title').textContent = post ? 'Editar post' : 'Nuevo post';
  document.getElementById('modal-body').innerHTML = `
  <div style="display:flex;flex-direction:column;gap:14px;padding:4px 0;">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <div>
        <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">Curso *</label>
        <select id="bp-curso" onchange="_blogActualizarRAs()"
          style="width:100%;padding:8px;border:1.5px solid #E0E0E0;border-radius:8px;background:var(--color-superficie,#fff);color:var(--color-texto-primario,#212121);box-sizing:border-box;">
          <option value="">— Selecciona —</option>${opsCursos}
        </select>
      </div>
      <div>
        <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">RA / Planificación *</label>
        <select id="bp-plan"
          style="width:100%;padding:8px;border:1.5px solid #E0E0E0;border-radius:8px;background:var(--color-superficie,#fff);color:var(--color-texto-primario,#212121);box-sizing:border-box;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <div>
        <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">Tipo *</label>
        <select id="bp-tipo"
          style="width:100%;padding:8px;border:1.5px solid #E0E0E0;border-radius:8px;background:var(--color-superficie,#fff);color:var(--color-texto-primario,#212121);box-sizing:border-box;">
          <option value="pc"       ${post?.tipo === 'pc'       ? 'selected' : ''}>🖥️ Tarea en PC</option>
          <option value="cuaderno" ${post?.tipo === 'cuaderno' ? 'selected' : ''}>📓 Tarea en cuaderno</option>
          <option value="info"     ${post?.tipo === 'info'     ? 'selected' : ''}>ℹ️ Información</option>
        </select>
      </div>
      <div>
        <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">Fecha límite</label>
        <input type="date" id="bp-fecha" value="${post?.fechaLimite || ''}"
          style="width:100%;padding:8px;border:1.5px solid #E0E0E0;border-radius:8px;background:var(--color-superficie,#fff);color:var(--color-texto-primario,#212121);box-sizing:border-box;">
      </div>
    </div>
    <div>
      <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">Título *</label>
      <input type="text" id="bp-titulo" value="${escapeHTML(post?.titulo || '')}"
        placeholder="Ej: Actividad 3 — Diagrama de flujo"
        style="width:100%;padding:8px;border:1.5px solid #E0E0E0;border-radius:8px;background:var(--color-superficie,#fff);color:var(--color-texto-primario,#212121);box-sizing:border-box;">
    </div>
    <div>
      <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">Instrucciones / Contenido *</label>
      <textarea id="bp-contenido" rows="5"
        placeholder="Escribe las instrucciones de la tarea o la información a compartir..."
        style="width:100%;padding:8px;border:1.5px solid #E0E0E0;border-radius:8px;resize:vertical;font-family:inherit;background:var(--color-superficie,#fff);color:var(--color-texto-primario,#212121);box-sizing:border-box;">${escapeHTML(post?.contenido || '')}</textarea>
    </div>
    <div>
      <label style="font-size:0.82rem;font-weight:600;color:#616161;display:block;margin-bottom:4px;">
        Archivos adjuntos <span style="font-weight:400;color:#9E9E9E;">(máx. 3 · 400 KB c/u)</span>
      </label>
      <input type="file" id="bp-adjunto-input" multiple onchange="_blogAgregarAdjuntos(event)" style="display:none;">
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:4px;">
        <button type="button" onclick="document.getElementById('bp-adjunto-input').click()"
          style="background:#F5F5F5;border:1.5px dashed #BDBDBD;border-radius:8px;padding:6px 14px;font-size:0.82rem;cursor:pointer;color:#616161;display:inline-flex;align-items:center;gap:4px;">
          <span class="material-icons" style="font-size:16px;">attach_file</span> Adjuntar
        </button>
        <div id="bp-adjuntos-lista" style="display:flex;gap:6px;flex-wrap:wrap;">
          ${window._blogTempAdjuntos.map((a, i) =>
            `<span class="blog-adjunto-chip">${escapeHTML(a.nombre)}
              <button onclick="_blogQuitarAdjunto(${i})" style="background:none;border:none;cursor:pointer;color:#C62828;padding:0 2px;line-height:1;">✕</button>
            </span>`).join('')}
        </div>
      </div>
    </div>
    <div style="display:flex;gap:8px;justify-content:flex-end;padding-top:8px;border-top:1px solid #E0E0E0;">
      <button class="btn-secundario" onclick="cerrarModalBtn()">Cancelar</button>
      <button class="btn-siguiente" onclick="_guardarPost('${id || ''}')">
        <span class="material-icons">save</span> Guardar
      </button>
    </div>
  </div>`;

  if (post) _blogActualizarRAs(post.planId);
  document.getElementById('modal-overlay').classList.remove('hidden');
}

function _blogActualizarRAs(selectedPlanId) {
  const cursoId = document.getElementById('bp-curso')?.value;
  const selPlan = document.getElementById('bp-plan');
  if (!selPlan) return;
  if (!cursoId) { selPlan.innerHTML = '<option value="">— Primero elige curso —</option>'; return; }
  const curso = calState.cursos[cursoId];
  if (!curso) return;
  const biblio = cargarBiblioteca().items || [];
  // Mantener orden de planIds para asignar RA1, RA2…
  const planIds = curso.planIds || [];
  ...
  selPlan.innerHTML = '<option value="">— Selecciona RA —</option>'
    + plans.map((pl, idx) => {
        ...
        if (raDesc && raDesc !== modulo) display += ' · ' + raDesc.substring(0, 35);
        return `<option value="${escapeHTML(pl.id)}" ${(selectedPlanId || '') === pl.id ? 'selected' : ''}>${escapeHTML(display)}</option>`;
      }).join('');
}

function _blogAgregarAdjuntos(event) {
  const files = Array.from(event.target.files || []);
  for (const file of files) {
    if ((window._blogTempAdjuntos || []).length >= 3) { mostrarToast('Máximo 3 archivos', 'warning'); break; }
    if (file.size > 400 * 1024) { mostrarToast(file.name + ' supera los 400 KB', 'warning'); continue; }
    const reader = new FileReader();
    reader.onload = function(e) {
      window._blogTempAdjuntos = window._blogTempAdjuntos || [];
      window._blogTempAdjuntos.push({ nombre: file.name, datos: e.target.result });
      _blogRefrescarListaAdjuntos();
    };
...
function _blogQuitarAdjunto(idx) {
  (window._blogTempAdjuntos = window._blogTempAdjuntos || []).splice(idx, 1);
  _blogRefrescarListaAdjuntos();
}

function _blogRefrescarListaAdjuntos() {
  const lista = document.getElementById('bp-adjuntos-lista');
  if (!lista) return;
  lista.innerHTML = (window._blogTempAdjuntos || []).map((a, i) =>
    `<span class="blog-adjunto-chip">${escapeHTML(a.nombre)}
      <button onclick="_blogQuitarAdjunto(${i})" style="background:none;border:none;cursor:pointer;color:#C62828;padding:0 2px;line-height:1;">✕</button>
    </span>`
  ).join('');
...
function _guardarPost(id) {
  const cursoId   = document.getElementById('bp-curso')?.value.trim();
  const planId    = document.getElementById('bp-plan')?.value.trim();
  const tipo      = document.getElementById('bp-tipo')?.value || 'info';
  const titulo    = document.getElementById('bp-titulo')?.value.trim();
  const contenido = document.getElementById('bp-contenido')?.value.trim();
  const fechaLimite = document.getElementById('bp-fecha')?.value || null;
  if (!cursoId)   { mostrarToast('Selecciona un curso', 'error'); return; }
  if (!planId)    { mostrarToast('Selecciona un RA', 'error'); return; }
  if (!titulo)    { mostrarToast('Escribe un título', 'error'); return; }
  if (!contenido) { mostrarToast('Escribe el contenido', 'error'); return; }
...
function eliminarPost(id) {
  ...
  const blog = cargarBlog();
  const post = (blog.posts || []).find(p => p.id === id);
  if (post?.publicado) _blogDespublicarEnFirestore(id);
...
function _blogPublicarEnFirestore(post) {
  if (!window.currentUser || typeof db === 'undefined') return;
  const uid = window.currentUser.uid;
  db.collection('public_blogs').doc(uid).collection('posts').doc(post.id)
    .set({ ...post })
    .catch(e => console.warn('Error publicando post en Firestore:', e));
}

function _blogDespublicarEnFirestore(postId) {
  if (!window.currentUser || typeof db === 'undefined') return;
  const uid = window.currentUser.uid;
  db.collection('public_blogs').doc(uid).collection('posts').doc(postId)
    .delete()
    .catch(e => console.warn('Error despublicando post:', e));
}

function copiarLinkBlog() {
  const filtro = document.getElementById('blog-filtro-curso')?.value || '';
...
function verEntregasPost(postId) {
  if (!window.currentUser || typeof db === 'undefined') return;
  const uid = window.currentUser.uid;
  document.getElementById('modal-title').textContent = 'Entregas de estudiantes';
  document.getElementById('modal-body').innerHTML = `
    <div style="text-align:center;padding:24px;">
      <span class="material-icons" style="font-size:36px;color:#BDBDBD;display:block;margin-bottom:8px;">hourglass_empty</span>
      <p style="color:#9E9E9E;font-size:0.85rem;">Cargando entregas...</p>
    </div>`;
  document.getElementById('modal-overlay').classList.remove('hidden');

  db.collection('public_blogs').doc(uid).collection('submissions')
    .where('postId', '==', postId)
    .orderBy('fecha', 'desc')
    .get()
    .then(snap => {
      ...
      if (!body) return;
      if (docs.length === 0) {
        body.innerHTML = '<div style="text-align:center;padding:24px;color:#9E9E9E;">'
          + '<span class="material-icons" style="font-size:40px;display:block;margin-bottom:8px;">inbox</span>'
          + '<p>Ningún estudiante ha entregado aún.</p></div>';
        return;
      }
      body.innerHTML = `<div style="font-size:0.82rem;color:#616161;margin-bottom:12px;">${docs.length} entrega(s)</div>`
        + docs.map((s, idx) => `
          <div style="border:1px solid #E0E0E0;border-radius:8px;padding:12px 14px;margin-bottom:10px;">
            <div style="font-weight:700;font-size:0.9rem;margin-bottom:4px;">${escapeHTML(s.nombre || 'Sin nombre')}</div>
            ${s.comentario ? `<div style="font-size:0.82rem;color:#424242;margin-bottom:8px;">${escapeHTML(s.comentario)}</div>` : ''}
            ${s.archivo
              ? `<button onclick="_descargarEntrega(window._blogEntregas[${idx}].archivo.datos,'${escapeHTML(s.archivo.nombre)}')"
                  style="background:#E3F2FD;border:1.5px solid #90CAF9;border-radius:6px;padding:5px 12px;font-size:0.78rem;cursor:pointer;color:#1565C0;display:inline-flex;align-items:center;gap:4px;margin-bottom:6px;">
                  <span class="material-icons" style="font-size:14px;">download</span> ${escapeHTML(s.archivo.nombre)}
                </button>`
              : '<span style="font-size:0.78rem;color:#9E9E9E;">Sin archivo adjunto</span>'}
            <div style="font-size:0.72rem;color:#9E9E9E;margin-top:4px;">${new Date(s.fecha).toLocaleString('es')}</div>
          </div>`).join('');
      window._blogEntregas = docs;
    })
    .catch(e => {
      const body = document.getElementById('modal-body');
      if (body) body.innerHTML = `<div style="color:#C62828;padding:16px;">Error: ${escapeHTML(e.message)}</div>`;
    });
}

function _descargarEntrega(datos, nombre) {
  const a = document.createElement('a');
  a.href = datos;
  a.download = nombre;
  a.click();
...
const TIMER_CIRCUM = 2 * Math.PI * 52; // ≈ 326.73
...
function _timerPreset(minutes) {
  if (_timerState.running) return;
  const secs = minutes * 60;
  _timerState.total     = secs;
  _timerState.remaining = secs;
  // Actualizar inputs
  const inMin = document.getElementById('timer-input-min');
  const inSeg = document.getElementById('timer-input-seg');
  if (inMin) inMin.value = minutes;
  if (inSeg) inSeg.value = 0;
  // Highlight preset activo
  document.querySelectorAll('.timer-preset-btn').forEach(b => {
    b.classList.toggle('activo', parseInt(b.textContent) === minutes);
  });
  _timerActualizarDisplay();
  _timerActualizarUI();
}
...
function _timerIniciar() {
  // Si no hay tiempo cargado, leer de los inputs
  if (_timerState.total === 0 || _timerState.remaining === 0) {
    const inMin = document.getElementById('timer-input-min');
    const inSeg = document.getElementById('timer-input-seg');
    const m = Math.max(0, parseInt(inMin?.value || '0'));
    const s = Math.max(0, Math.min(59, parseInt(inSeg?.value || '0')));
    const total = m * 60 + s;
    if (total <= 0) return;
    _timerState.total     = total;
    _timerState.remaining = total;
  }
  if (_timerState.remaining <= 0) return;
  _timerState.running  = true;
  _timerState.interval = setInterval(_timerTick, 1000);
  _timerActualizarDisplay();
  _timerActualizarUI();
}

function _timerParar() {
  _timerState.running = false;
  clearInterval(_timerState.interval);
  _timerState.interval = null;
  _timerActualizarUI();
}
...
function _timerReset() {
  ...
  if (fab)  { fab.classList.remove('timer-fab-running', 'timer-fab-fin'); }
  if (ring) { ring.classList.remove('timer-running', 'timer-fin'); }
  // Limpiar preset activo
  document.querySelectorAll('.timer-preset-btn').forEach(b => b.classList.remove('activo'));
  // Limpiar inputs
  const inMin = document.getElementById('timer-input-min');
  const inSeg = document.getElementById('timer-input-seg');
...
function _timerFin() {
  ...
  const fab  = document.getElementById('timer-fab');
  const ring = document.getElementById('timer-ring-prog');
  if (fab)  { fab.classList.remove('timer-fab-running'); fab.classList.add('timer-fab-fin'); }
  if (ring) { ring.classList.remove('timer-running'); ring.classList.add('timer-fin'); }
  _timerActualizarDisplay();
  _timerActualizarUI();
  // Quitar clases de fin después de 6 s
  setTimeout(() => {
    if (fab)  fab.classList.remove('timer-fab-fin');
    if (ring) ring.classList.remove('timer-fin');
  }, 6000);
}

function _timerActualizarDisplay() {
  const rem  = Math.max(0, _timerState.remaining);
  const tot  = Math.max(1, _timerState.total);
  ...
  if (disp) disp.textContent = tot === 1 && rem === 0 ? '00:00' : txt;

  // Anillo SVG
  const ring = document.getElementById('timer-ring-prog');
  if (ring) {
    const frac   = _timerState.total > 0 ? rem / tot : 0;
    const offset = TIMER_CIRCUM * (1 - frac);
    ring.style.strokeDashoffset = offset;
  }

  // FAB: icono vs tiempo
  const fabIcon = document.getElementById('timer-fab-icon');
  const fabTime = document.getElementById('timer-fab-time');
  if (_timerState.total > 0) {
    if (fabIcon) fabIcon.classList.add('hidden');
    if (fabTime) { fabTime.classList.remove('hidden'); fabTime.textContent = txt; }
  } else {
    if (fabIcon) fabIcon.classList.remove('hidden');
    if (fabTime) fabTime.classList.add('hidden');
  }
}

function _timerActualizarUI() {
  const playBtn = document.getElementById('timer-btn-play');
  const fab     = document.getElementById('timer-fab');
  const ring    = document.getElementById('timer-ring-prog');
  if (playBtn) {
    const icon = playBtn.querySelector('.material-icons');
    if (icon) icon.textContent = _timerState.running ? 'pause' : 'play_arrow';
  }
  if (fab) {
    if (_timerState.running) {
      fab.classList.add('timer-fab-running');
      fab.classList.remove('timer-fab-fin');
    } else {
      fab.classList.remove('timer-fab-running');
    }
  }
  if (ring) {
    ring.classList.toggle('timer-running', _timerState.running);
  }
}
...
function _timerBeep() {
  try {
    ...
    beepOnce(ctx.currentTime);
    beepOnce(ctx.currentTime + 0.45);
    beepOnce(ctx.currentTime + 0.90);
...
async function abrirHistorialSesiones() {
  const user = window.currentUser;
  if (!user) return;
  document.getElementById('sesiones-overlay')?.classList.remove('hidden');
  document.getElementById('sesiones-loading')?.classList.remove('hidden');
  document.getElementById('sesiones-lista')?.classList.add('hidden');
  ...
  try {
    ...
    if (snap.empty) {
      ...
      snap.forEach(doc => {
        ...
        const fechaStr = ts ? ts.toLocaleDateString('es-ES', { weekday:'short', day:'2-digit', month:'short', year:'numeric' }) : '—';
        const horaStr  = ts ? ts.toLocaleTimeString('es-ES', { hour:'2-digit', minute:'2-digit' }) : '—';

        const deviceIcon = d.device === 'Escritorio' ? 'computer' : 'smartphone';
        const browserIcon = { Chrome:'language', Firefox:'language', Safari:'language', Edge:'language', Opera:'language' }[d.browser] || 'public';

        html += `<div class="sesion-entry">
          <div class="sesion-entry-icon"><span class="material-icons">${deviceIcon}</span></div>
          <div class="sesion-entry-info">
            <div class="sesion-entry-titulo">${escapeHTML(d.browser)} · ${escapeHTML(d.os)}</div>
            <div class="sesion-entry-meta">${escapeHTML(d.device)} &nbsp;·&nbsp; ${escapeHTML(d.resolution)} &nbsp;·&nbsp; ${escapeHTML(d.language)}</div>
            <div class="sesion-entry-fecha">${fechaStr} &nbsp;–&nbsp; ${horaStr}</div>
          </div>
        </div>`;
      });
      lista.innerHTML = html;
    ...
    document.getElementById('sesiones-loading')?.classList.add('hidden');
    lista.classList.remove('hidden');
  } catch (e) {
    const lista = document.getElementById('sesiones-lista');
    if (lista) {
      lista.innerHTML = '<p style="text-align:center;color:#C62828;padding:32px 0;">Error al cargar el historial.</p>';
      lista.classList.remove('hidden');
    }
    document.getElementById('sesiones-loading')?.classList.add('hidden');
  }
...
function cerrarHistorialSesiones() {
  document.getElementById('sesiones-overlay')?.classList.add('hidden');
}

async function limpiarHistorialSesiones() {
  const user = window.currentUser;
  if (!user) return;
  ...
  try {
    ...
    await batch.commit();
    registrarCambio('Historial de sesiones eliminado');
    abrirHistorialSesiones(); // re-carga (mostrará vacío)
  } catch (e) {
    mostrarToast('Error al limpiar historial', 'error');
  }
}

// ================================================================
// MÓDULO REPORTES ACADÉMICOS Y DISCIPLINARIOS
// ================================================================
const REPORTES_KEY = 'planificadorRA_reportes_v1';
let _repEstId   = null;
let _repCursoId = null;

function cargarReportes() {
  try { return JSON.parse(localStorage.getItem(REPORTES_KEY)) || {}; } catch(e) { return {}; }
}
...
function abrirReportes(estId, cursoId) {
  _repEstId   = estId;
  _repCursoId = cursoId;
  const curso = calState.cursos[cursoId];
  const est   = curso && curso.estudiantes.find(e => e.id === estId);
  const elNombre = document.getElementById('rep-est-nombre');
  if (elNombre) elNombre.textContent = est ? est.nombre : '—';
  renderizarReportes(estId);
  document.getElementById('reportes-overlay')?.classList.remove('hidden');
}

function cerrarReportesBtn() {
  document.getElementById('reportes-overlay')?.classList.add('hidden');
  _repEstId = null; _repCursoId = null;
}
function cerrarReportes(e) {
  if (e.target.id === 'reportes-overlay') cerrarReportesBtn();
}

function renderizarReportes(estId) {
  const body = document.getElementById('rep-body');
  if (!body) return;
  const reportes    = _getReportesEst(estId);
  const curso       = calState.cursos[_repCursoId];
  const est         = curso && curso.estudiantes.find(e => e.id === estId);
  ...
  let listaHtml = '';
  if (reportes.length) {
    listaHtml = '<div style="padding:16px 20px 0;">' +
      '<div style="font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;color:#546E7A;margin-bottom:10px;">Reportes guardados</div>';
    reportes.forEach(r => {
      const fecha = r.fechaReporte
        ? new Date(r.fechaReporte + 'T12:00:00').toLocaleDateString('es-ES', {weekday:'short',day:'2-digit',month:'short',year:'numeric'})
        : '—';
      ...
      listaHtml +=
        ...
        'border:1px solid var(--color-borde,#E0E0E0);margin-bottom:8px;background:var(--color-fondo-card,#FAFAFA);">' +
          ...
          '</button>' +
          '<button class="btn-icon-sm" title="Eliminar reporte" onclick="eliminarReporte(\'' + r.id + '\',\'' + estId + '\')" style="color:#C62828;">' +
            '<span class="material-icons" style="font-size:16px;">delete</span>' +
          '</button>' +
        '</div>';
    });
    listaHtml += '</div><hr style="border:none;border-top:1px solid var(--color-borde,#E0E0E0);margin:0;">';
  }
  ...
  const formHtml =
    ...
    '<div style="font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;color:#546E7A;margin-bottom:12px;">Nuevo reporte</div>' +

    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">' +
      '<div><label style="' + lbl + '">Nombre del estudiante</label>' +
        '<input id="rep-f-nombre" style="' + fld + '" type="text" value="' + escapeHTML(est ? est.nombre : '') + '"></div>' +
      '<div><label style="' + lbl + '">Grado / Curso</label>' +
        '<input id="rep-f-grado" style="' + fld + '" type="text" value="' + escapeHTML(cursoNombre) + '"></div>' +
    '</div>' +

    '<label style="' + lbl + '">Fecha</label>' +
    '<input id="rep-f-fecha" style="' + fld + '" type="date" value="' + hoy + '">' +

    '<label style="' + lbl + '">Detalles del evento *</label>' +
    '<textarea id="rep-f-detalles" style="' + ta + '" placeholder="Describe el evento o situación académica/disciplinaria..."></textarea>' +

    '<label style="' + lbl + '">Medidas trabajadas por el docente en el aula</label>' +
    '<textarea id="rep-f-medidas" style="' + ta + '" placeholder="Acciones y estrategias aplicadas por el docente..."></textarea>' +

    '<label style="' + lbl + '">Si la falta es repetitiva, amerita seguimiento de Coordinación o Psicología — especificar:</label>' +
    '<textarea id="rep-f-seguimiento" style="' + ta + '" placeholder="Describir si aplica seguimiento institucional..."></textarea>' +

    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:4px;">' +
      '<div><label style="' + lbl + '">Firma estudiante (nombre)</label>' +
        '<input id="rep-f-firma-est" style="' + fld + '" type="text" placeholder="Nombre del estudiante"></div>' +
      '<div><label style="' + lbl + '">Número celular estudiante</label>' +
        '<input id="rep-f-cel-est" style="' + fld + '" type="tel" placeholder="Ej: 0999-000-000"></div>' +
    '</div>' +

    '<hr style="border:none;border-top:1px dashed var(--color-borde,#BDBDBD);margin:16px 0 4px;">' +
    '<div style="font-size:0.75rem;font-weight:700;color:#546E7A;margin-bottom:4px;">Firmas institucionales</div>' +

    '<label style="' + lbl + '">Firma del docente</label>' +
    '<input id="rep-f-firma-doc" style="' + fld + '" type="text" placeholder="Nombre del docente">' +

    '<label style="' + lbl + '">Firma de coordinación pedagógica</label>' +
    '<input id="rep-f-firma-coord" style="' + fld + '" type="text" placeholder="Nombre del coordinador">' +

    '<label style="' + lbl + '">Firma de psicología</label>' +
    '<input id="rep-f-firma-psico" style="' + fld + '" type="text" placeholder="Nombre del psicólogo (si aplica)">' +

    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">' +
      '<div><label style="' + lbl + '">Firma del padre, madre o tutor</label>' +
        '<input id="rep-f-firma-tutor" style="' + fld + '" type="text" placeholder="Nombre del tutor"></div>' +
      '<div><label style="' + lbl + '">Teléfono del padre, madre o tutor</label>' +
        '<input id="rep-f-tel-tutor" style="' + fld + '" type="tel" placeholder="Ej: 0988-000-000"></div>' +
    '</div>' +

    '<div id="rep-error" style="color:#C62828;font-size:0.82rem;margin-top:8px;min-height:18px;"></div>' +

    '<button class="auth-btn-primary" style="margin-top:14px;width:100%;" onclick="guardarReporteNuevo(\'' + estId + '\')">' +
      '<span class="material-icons">save</span> Guardar reporte' +
    '</button>' +
    '</div>';

  body.innerHTML = listaHtml + formHtml;
}

function guardarReporteNuevo(estId) {
  const detalles = (document.getElementById('rep-f-detalles')?.value || '').trim();
  const errEl = document.getElementById('rep-error');
  if (!detalles) {
    if (errEl) errEl.textContent = 'Los detalles del evento son obligatorios.';
    return;
  }
  ...
  const nuevo = {
    ...
    medidasDocente:        (document.getElementById('rep-f-medidas')?.value || '').trim(),
    seguimientoRepetitiva: (document.getElementById('rep-f-seguimiento')?.value || '').trim(),
    firmaEstudiante:       (document.getElementById('rep-f-firma-est')?.value || '').trim(),
    celularEstudiante:     (document.getElementById('rep-f-cel-est')?.value || '').trim(),
    firmaDocente:          (document.getElementById('rep-f-firma-doc')?.value || '').trim(),
    firmaCoordenacion:     (document.getElementById('rep-f-firma-coord')?.value || '').trim(),
    firmaPsicologia:       (document.getElementById('rep-f-firma-psico')?.value || '').trim(),
    firmaTutor:            (document.getElementById('rep-f-firma-tutor')?.value || '').trim(),
    telefonoTutor:         (document.getElementById('rep-f-tel-tutor')?.value || '').trim()
  };

  const data = cargarReportes();
  if (!data[estId]) data[estId] = [];
  data[estId].push(nuevo);
...
function eliminarReporte(id, estId) {
  ...
  const data = cargarReportes();
  if (data[estId]) data[estId] = data[estId].filter(r => r.id !== id);
  guardarReportes(data);
...
function imprimirReporte(id, estId) {
  ...
  const fecha = rep.fechaReporte
    ? new Date(rep.fechaReporte + 'T12:00:00').toLocaleDateString('es-ES', { day:'2-digit', month:'long', year:'numeric' })
    : '—';

  const esc = escapeHTML;

  const html = '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">' +
    '<title>Reporte — ' + esc(rep.estudianteNombre) + '</title>' +
    '<style>body{font-family:Arial,sans-serif;margin:18mm 18mm 14mm;font-size:11pt;}' +
    'table{width:100%;border-collapse:collapse;}' +
    'td{border:1px solid #000;padding:5px 8px;font-size:11pt;vertical-align:top;}' +
    'h3{text-align:center;font-size:13pt;margin-bottom:14px;}' +
    '.area{min-height:70px;border:1px solid #000;padding:5px 6px;font-size:11pt;margin-top:3px;}' +
    'b{font-size:10pt;}' +
    '@media print{.no-print{display:none!important;}}</style></head><body>' +
    '<div class="no-print" style="text-align:center;margin-bottom:14px;">' +
      '<button onclick="window.print()" style="padding:9px 26px;background:#1565C0;color:#fff;border:none;border-radius:6px;font-size:13px;cursor:pointer;">' +
        'Imprimir / Guardar PDF' +
      '</button>' +
    '</div>' +
    '<h3>Reporte acad&eacute;mico y/o situaciones especiales de disciplina.</h3>' +
    '<table>' +
      '<tr><td style="width:43%;"><b>Nombre y apellido de estudiante</b></td><td>' + esc(rep.estudianteNombre) + '</td></tr>' +
      '<tr><td><b>Grado que cursa el estudiante</b></td><td>' + esc(rep.gradoCursa) + '</td></tr>' +
      '<tr><td><b>Fecha</b></td><td>' + esc(fecha) + '</td></tr>' +
      '<tr><td colspan="2"><b>Detalles del evento:</b><div class="area">' + esc(rep.detallesEvento).replace(/\n/g,'<br>') + '</div></td></tr>' +
      '<tr><td colspan="2"><b>Medidas trabajadas por el docente en el aula:</b><div class="area">' + esc(rep.medidasDocente || '').replace(/\n/g,'<br>') + '</div></td></tr>' +
      '<tr><td colspan="2"><b>Si la falta cometida por el estudiante es repetitiva, la cual amerita seguimiento de Coordinaci&oacute;n o psicolog&iacute;a, especificar la misma:</b>' +
        '<div class="area">' + esc(rep.seguimientoRepetitiva || '').replace(/\n/g,'<br>') + '</div></td></tr>' +
      '<tr><td><b>Firma estudiante</b><br><br>' + esc(rep.firmaEstudiante) + '</td><td></td></tr>' +
      '<tr><td><b>N&uacute;mero celular</b></td><td>' + esc(rep.celularEstudiante) + '</td></tr>' +
    '</table>' +
    '<table style="margin-top:16px;">' +
      '<tr><td style="width:43%;"><b>Firma del docente</b><br><br>' + esc(rep.firmaDocente) + '</td><td></td></tr>' +
      '<tr><td><b>Firma de coordinaci&oacute;n pedag&oacute;gica</b><br><br>' + esc(rep.firmaCoordenacion) + '</td><td></td></tr>' +
      '<tr><td><b>Firma de psicolog&iacute;a</b><br><br>' + esc(rep.firmaPsicologia) + '</td><td></td></tr>' +
      '<tr><td><b>Firma del padre, madre o tutor</b><br><br>' + esc(rep.firmaTutor) + '</td><td></td></tr>' +
      '<tr><td><b>Tel&eacute;fono del padre, madre o tutor</b></td><td>' + esc(rep.telefonoTutor) + '</td></tr>' +
    '</table>' +
...
// ================================================================
// MODO PRESENTACIÓN — Vista limpia para proyector/pizarra
// ================================================================
const _PRES_HORAS = { 1:480, 2:530, 3:600, 4:650, 5:750, 6:800, 7:860, 8:910 };
const _PRES_PERIODOS_LABEL = {
  1:'8:00 – 8:50 AM', 2:'8:50 – 9:40 AM', 3:'10:00 – 10:50 AM', 4:'10:50 – 11:40 AM',
  5:'12:30 – 1:20 PM', 6:'1:20 – 2:10 PM', 7:'2:20 – 3:10 PM', 8:'3:10 – 4:00 PM'
};

let _presState = {
  cursoId: null, periodoId: null, periodFinMin: null,
  clockInterval: null,
  timerTotal: 0, timerRem: 0, timerRunning: false, timerInterval: null
};
...
function abrirModoPresent() {
  ...
  _presIniciarReloj();
  registrarCambio('Modo presentación iniciado');
}

function cerrarModoPresent() {
  clearInterval(_presState.clockInterval);
  clearInterval(_presState.timerInterval);
  _presState = { cursoId:null, periodoId:null, periodFinMin:null,
    clockInterval:null, timerTotal:0, timerRem:0, timerRunning:false, timerInterval:null };
  document.getElementById('pres-overlay')?.classList.add('hidden');
  if (document.fullscreenElement) document.exitFullscreen().catch(()=>{});
}

function _presDetectarClase() {
  const horario = cargarHorario();
  if (!horario || !horario.length) return null;
  const now = new Date();
  const diaSem = now.getDay() - 1; // 0=Lun...4=Vie
...
function _presCargarCurso(cursoId, periodoId) {
  ...
  if (periodoId && _PRES_HORAS[periodoId] !== undefined) {
    _presState.periodFinMin = _PRES_HORAS[periodoId] + 50;
  } else {
  ...
  if (elNombre) elNombre.textContent = curso ? curso.nombre : '—';
  if (elPer) {
    const partes = [];
    if (periodoId) partes.push('Período ' + periodoId + ' · ' + (_PRES_PERIODOS_LABEL[periodoId] || ''));
    if (slot && slot.aula) partes.push('Aula ' + slot.aula);
    if (slot && slot.seccion) partes.push(slot.seccion);
    elPer.textContent = partes.join('  ·  ');
  }

  // Objetivo
  const objWrap = document.getElementById('pres-objetivo-wrap');
  if (objWrap) {
    const ra = plan && plan.ra;
    const desc = (ra && ra.descripcion) ? ra.descripcion : '';
    const modulo = (plan && plan.datosGenerales && plan.datosGenerales.moduloFormativo) ? plan.datosGenerales.moduloFormativo : '';
    objWrap.innerHTML =
      '<div class="pres-section-label">Objetivo de aprendizaje</div>' +
      (modulo ? '<div style="font-size:0.78rem;color:#90CAF9;margin-bottom:6px;font-weight:600;">' + escapeHTML(modulo) + '</div>' : '') +
      '<div id="pres-objetivo-texto">' +
        (desc ? escapeHTML(desc) : '<span style="opacity:0.4;font-style:italic;">Sin objetivo definido en este plan</span>') +
      '</div>';
  }

  // Secuencia didáctica (primer EC)
  const seqWrap = document.getElementById('pres-secuencia-wrap');
  if (seqWrap) {
    const ecs = plan && plan.elementosCapacidad;
    const ec  = ecs && ecs.length ? ecs[0] : null;
    const seq = ec && ec.secuencia;
    if (seq && (seq.apertura || seq.desarrollo || seq.cierre)) {
      const bloques = [
        { label:'Apertura',    texto: seq.apertura   || '', color:'#FFD54F' },
        { label:'Desarrollo',  texto: seq.desarrollo || '', color:'#80CBC4' },
        { label:'Cierre',      texto: seq.cierre     || '', color:'#EF9A9A' }
      ];
      let html = '<div class="pres-section-label" style="margin-top:24px;">Secuencia didáctica</div>';
      if (ec.enunciado) html += '<div style="font-size:0.78rem;color:#90CAF9;margin-bottom:8px;">' + escapeHTML(ec.enunciado) + '</div>';
      bloques.forEach(b => {
        if (!b.texto.trim()) return;
        html += '<div class="pres-seq-blk">' +
          '<div class="pres-seq-label" style="color:' + b.color + ';">' + b.label + '</div>' +
          '<div class="pres-seq-texto">' + escapeHTML(b.texto) + '</div>' +
          '</div>';
      });
    ...
    if (acts.length) {
      ...
      acts.forEach(a => {
        ...
        html += '<div class="pres-act-item' + (esHoy ? ' hoy' : '') + '">' +
          ...
          '<div>' +
            '<div class="pres-act-texto">' + escapeHTML(a.enunciado || a.titulo || '—') +
              (esHoy ? ' <span class="pres-act-badge-hoy">HOY</span>' : '') +
            '</div>' +
            (fechaStr ? '<div class="pres-act-fecha">' + fechaStr + '</div>' : '') +
          '</div>' +
          '</div>';
      });
      actWrap.innerHTML = html;
    } else {
      actWrap.innerHTML = '<div class="pres-section-label">Actividades del plan</div>' +
        '<p style="font-size:0.85rem;color:rgba(255,255,255,0.3);font-style:italic;">Sin actividades en este plan</p>';
    }
  }
...
function _presRenderizarVacio() {
  const elNombre = document.getElementById('pres-curso-nombre');
  const elPer    = document.getElementById('pres-periodo-info');
  if (elNombre) elNombre.textContent = 'Selecciona una clase';
  if (elPer)    elPer.textContent    = 'Sin clase activa detectada';
  const objWrap = document.getElementById('pres-objetivo-wrap');
...
function _presRenderizarTimer() {
  const box = document.getElementById('pres-timer-box');
  if (!box) return;
  const rem = _presState.timerRem;
  const mm = String(Math.floor(rem / 60)).padStart(2, '0');
  const ss = String(rem % 60).padStart(2, '0');
  const finClass = rem === 0 && _presState.timerTotal > 0 ? ' fin' : '';
  box.innerHTML =
    '<div class="pres-section-label">Temporizador</div>' +
    '<div id="pres-timer-display" class="' + finClass + '">' + mm + ':' + ss + '</div>' +
    '<div class="pres-timer-row">' +
      '<button class="pres-t-btn" id="pres-btn-play" onclick="_presTimerToggle()">' +
        '<span class="material-icons" style="font-size:18px;vertical-align:middle;">' +
          (_presState.timerRunning ? 'pause' : 'play_arrow') + '</span>' +
      '</button>' +
      '<button class="pres-t-btn" onclick="_presTimerReset()">' +
        '<span class="material-icons" style="font-size:18px;vertical-align:middle;">replay</span>' +
      '</button>' +
    '</div>' +
    '<div class="pres-timer-row" style="margin-top:8px;">' +
      [5,10,15,20].map(m =>
        '<button class="pres-t-btn" onclick="_presTimerAgregar(' + m + ')">+' + m + 'm</button>'
      ).join('') +
    '</div>' +
    '<div id="pres-fin-count" style="font-size:0.78rem;color:#80DEEA;margin-top:10px;min-height:18px;"></div>';
  _presActualizarFinPeriodo();
}

function _presActualizarTimerDisplay() {
  const el = document.getElementById('pres-timer-display');
  if (!el) return;
  const rem = _presState.timerRem;
  const mm = String(Math.floor(rem / 60)).padStart(2, '0');
  const ss = String(rem % 60).padStart(2, '0');
  el.textContent = mm + ':' + ss;
  el.className = (rem === 0 && _presState.timerTotal > 0) ? 'fin' : '';
  // Actualizar icono play/pause
  const btnPlay = document.getElementById('pres-btn-play');
  if (btnPlay) {
    const ico = btnPlay.querySelector('.material-icons');
    if (ico) ico.textContent = _presState.timerRunning ? 'pause' : 'play_arrow';
  }
}

function _presTimerToggle() {
  if (_presState.timerRunning) _presTimerParar();
  else _presTimerIniciar();
}

function _presTimerIniciar() {
  if (_presState.timerRem <= 0) return;
  _presState.timerRunning = true;
  _presState.timerInterval = setInterval(_presTimerTick, 1000);
  _presActualizarTimerDisplay();
}

function _presTimerParar() {
  _presState.timerRunning = false;
  clearInterval(_presState.timerInterval);
  _presState.timerInterval = null;
  _presActualizarTimerDisplay();
}

function _presTimerReset() {
  _presTimerParar();
  _presState.timerRem   = _presState.timerTotal;
  _presState.timerRunning = false;
  _presActualizarTimerDisplay();
}

function _presTimerAgregar(min) {
  _presState.timerTotal += min * 60;
  _presState.timerRem   += min * 60;
  _presActualizarTimerDisplay();
}

function _presTimerTick() {
  if (_presState.timerRem > 0) {
    _presState.timerRem--;
    _presActualizarTimerDisplay();
  } else {
    _presTimerParar();
    // parpadeo visual (ya lo hace la clase .fin en CSS)
    _presActualizarTimerDisplay();
  }
}

function _presIniciarReloj() {
  clearInterval(_presState.clockInterval);
  const tick = () => {
  ...
  tick();
  _presState.clockInterval = setInterval(tick, 1000);
}

function _presActualizarFinPeriodo() {
  const el = document.getElementById('pres-fin-count');
  if (!el) return;
  if (_presState.periodFinMin === null) { el.textContent = ''; return; }
  const now = new Date();
  const minActual = now.getHours() * 60 + now.getMinutes() + now.getSeconds() / 60;
  const diff = _presState.periodFinMin - minActual;
  if (diff <= 0) {
    el.textContent = 'Período finalizado';
    return;
  }
  const mm = Math.floor(diff);
  const ss = Math.round((diff - mm) * 60);
  el.textContent = 'Fin del período en ' + String(mm).padStart(2,'0') + ':' + String(ss).padStart(2,'0');
}

function _presFullscreen() {
...
function abrirPresPicker() {
  const body = document.getElementById('pres-picker-body');
  if (!body) return;
  const cursos = Object.values(calState.cursos || {});
  if (!cursos.length) {
    body.innerHTML = '<p style="text-align:center;color:#757575;padding:24px 0;">No hay cursos creados aún.</p>';
  } else {
    // Detectar clase activa para resaltarla
    const detectado = _presDetectarClase();
    let html = '';
    cursos.forEach(c => {
      const esActual = detectado && detectado.cursoId === c.id;
      html += '<button onclick="_presElegirCurso(\'' + c.id + '\')" style="' +
  ...
  document.getElementById('pres-picker-overlay')?.classList.remove('hidden');
}

function cerrarPresPicker() {
  document.getElementById('pres-picker-overlay')?.classList.add('hidden');
}

function _presElegirCurso(cursoId) {
  cerrarPresPicker();
  // Intentar detectar el período actual para ese curso
  const detectado = _presDetectarClase();
...
const CAL_ESC_KEY = 'planificadorRA_calendario_escolar_v1';
const CAL_ESC_MESES = [
  'agosto','septiembre','octubre','noviembre','diciembre',
  'enero','febrero','marzo','abril','mayo','junio'
];
const CAL_ESC_MESES_LABEL = {
  agosto:'Agosto', septiembre:'Septiembre', octubre:'Octubre',
  noviembre:'Noviembre', diciembre:'Diciembre', enero:'Enero',
  febrero:'Febrero', marzo:'Marzo', abril:'Abril', mayo:'Mayo', junio:'Junio'
};
...
function _calEscDatosActuales() {
  return (_calEsc.modo === 'personal' && _calEsc.personalDatos)
    ? _calEsc.personalDatos
...
// ── Cargar / guardar calendario personal del usuario ─────────────────
function _calEscCargarPersonal() {
  try {
    const raw = localStorage.getItem(CAL_ESC_KEY);
    _calEsc.personalDatos = raw ? JSON.parse(raw) : null;
  } catch (e) { _calEsc.personalDatos = null; }
}

function _calEscGuardarPersonal() {
  const datos = _calEsc.personalDatos || _calEscDatosVacios();
  localStorage.setItem(CAL_ESC_KEY, JSON.stringify(datos));
  if (window._syncFirebase) _syncFirebase('calendario_escolar', datos);
}

// ── Abrir panel ──────────────────────────────────────────────────────
async function abrirCalendarioEscolar() {
  _mostrarPanel('panel-calendario-escolar');
  _calEsc.cargando = true;
  const c = document.getElementById('cal-escolar-contenido');
  ...
  if (_calEscEsAdmin()) {
    ...
    _calEsc.modo = _calEsc.personalDatos ? 'personal' : 'admin';
  }
...
function _calEscRenderizar() {
  ...
  if (badge) badge.style.display = _calEscEsAdmin() ? 'inline' : 'none';
}

function _calEscRenderizarBanner() {
  const banner = document.getElementById('cal-escolar-banner');
  const txt    = document.getElementById('cal-escolar-banner-txt');
  const icon   = document.getElementById('cal-escolar-banner-icon');
  const btns   = document.getElementById('cal-escolar-banner-btns');
  if (!banner || !txt || !btns) return;
  ...
  if (_calEscEsAdmin()) {
    ...
    btns.innerHTML   = `<button onclick="_calEscGuardarAdmin()"
      ...
      <span class="material-icons" style="font-size:14px;">cloud_upload</span> Publicar cambios
    </button>`;
  } else if (_calEsc.modo === 'admin') {
    banner.style.cssText += ';background:#E3F2FD;color:#0D47A1;border-color:#90CAF9;';
    icon.textContent = 'info';
    txt.textContent  = 'Estás viendo el calendario oficial del centro. Puedes usarlo o crear el tuyo propio.';
    btns.innerHTML   = `<button onclick="_calEscUsarMiCalendario()"
      style="background:#1565C0;color:#fff;border:none;border-radius:20px;padding:6px 14px;
             font-size:0.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;">
      <span class="material-icons" style="font-size:14px;">edit_calendar</span> Usar mi propio calendario
    </button>`;
  } else {
    banner.style.cssText += ';background:#FFF8E1;color:#E65100;border-color:#FFD54F;';
    icon.textContent = 'edit_calendar';
    txt.textContent  = 'Estás usando tu propio calendario. Los cambios solo te afectan a ti.';
    btns.innerHTML   = `<button onclick="_calEscRestaurarAdmin()"
      style="background:#E65100;color:#fff;border:none;border-radius:20px;padding:6px 14px;
             font-size:0.78rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;">
      <span class="material-icons" style="font-size:14px;">restore</span> Restaurar calendario oficial
    </button>`;
  }
}

function _calEscRenderizarTabs() {
  const tabs = document.getElementById('cal-escolar-tabs');
  if (!tabs) return;
  tabs.innerHTML = CAL_ESC_MESES.map(m => `
    <button onclick="_calEscSeleccionarMes('${m}')"
      style="padding:7px 14px;border-radius:20px;cursor:pointer;transition:all .15s;
             border:1.5px solid ${_calEsc.mesActivo===m?'#1565C0':'#E0E0E0'};
             background:${_calEsc.mesActivo===m?'#1565C0':'#fff'};
             color:${_calEsc.mesActivo===m?'#fff':'#546E7A'};
             font-size:0.8rem;font-weight:${_calEsc.mesActivo===m?'700':'500'};">
      ${CAL_ESC_MESES_LABEL[m]}
    </button>`).join('');
}

function _calEscSeleccionarMes(mes) {
  _calEsc.mesActivo = mes;
  _calEscRenderizarTabs();
  _calEscRenderizarMes();
}

// ── Render del mes activo ────────────────────────────────────────────
function _calEscRenderizarMes() {
  const c = document.getElementById('cal-escolar-contenido');
  if (!c) return;
  const datos   = _calEscDatosActuales();
  const mes     = _calEsc.mesActivo;
  const mesDatos = datos.meses?.[mes] || { actividades: [], efemerides: [] };
  const editable = _calEscEsAdmin() || _calEsc.modo === 'personal';

  c.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:4px;">
      <div style="background:#F9FAFB;border-radius:12px;padding:16px;border:1.5px solid #E0E0E0;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
          <h3 style="margin:0;font-size:0.95rem;font-weight:700;color:#212121;display:flex;align-items:center;gap:6px;">
            <span class="material-icons" style="font-size:18px;color:#1565C0;">task_alt</span>Actividades
          </h3>
          ${editable ? `<button onclick="_calEscAgregarActividad('${mes}')"
            style="background:#1565C0;color:#fff;border:none;border-radius:20px;padding:5px 12px;
                   font-size:0.75rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:4px;">
            <span class="material-icons" style="font-size:14px;">add</span> Agregar
          </button>` : ''}
        </div>
        ${_calEscRenderListaActividades(mes, mesDatos.actividades, editable)}
      </div>

      <div style="background:#F9FAFB;border-radius:12px;padding:16px;border:1.5px solid #E0E0E0;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
          <h3 style="margin:0;font-size:0.95rem;font-weight:700;color:#212121;display:flex;align-items:center;gap:6px;">
            <span class="material-icons" style="font-size:18px;color:#6A1B9A;">auto_stories</span>Efemérides
          </h3>
          ${editable ? `<button onclick="_calEscAgregarEfemeride('${mes}')"
            style="background:#6A1B9A;color:#fff;border:none;border-radius:20px;padding:5px 12px;
                   font-size:0.75rem;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:4px;">
            <span class="material-icons" style="font-size:14px;">add</span> Agregar
          </button>` : ''}
        </div>
        ${_calEscRenderListaEfemerides(mes, mesDatos.efemerides, editable)}
      </div>
    </div>`;
}

function _calEscRenderListaActividades(mes, lista, editable) {
  if (!lista || !lista.length) return `<p style="color:#9E9E9E;font-size:0.82rem;text-align:center;padding:16px 0;">Sin actividades registradas.</p>`;
  return lista.map((a, i) => `
    <div style="display:flex;align-items:flex-start;gap:8px;padding:8px 0;border-bottom:1px solid #EEEEEE;">
      <span class="material-icons" style="font-size:15px;color:#1565C0;margin-top:2px;flex-shrink:0;">check_circle</span>
      <span style="flex:1;font-size:0.85rem;color:#37474F;word-break:break-word;">${escapeHTML(a.texto)}</span>
      ${editable ? `
        <button onclick="_calEscEditarActividad('${mes}',${i})" title="Editar"
          style="background:none;border:none;cursor:pointer;color:#78909C;padding:2px;flex-shrink:0;">
          <span class="material-icons" style="font-size:15px;">edit</span>
        </button>
        <button onclick="_calEscEliminarActividad('${mes}',${i})" title="Eliminar"
          style="background:none;border:none;cursor:pointer;color:#E53935;padding:2px;flex-shrink:0;">
  ...
  return [...lista].sort((a, b) => (a.dia||0)-(b.dia||0)).map((ef, i) => `
    <div style="display:flex;align-items:flex-start;gap:8px;padding:8px 0;border-bottom:1px solid #EEEEEE;">
      ...
      ${editable ? `
        ...
        <button onclick="_calEscEliminarEfemeride('${mes}',${i})" title="Eliminar"
          style="background:none;border:none;cursor:pointer;color:#E53935;padding:2px;flex-shrink:0;">
          <span class="material-icons" style="font-size:15px;">delete</span>
        </button>` : ''}
    </div>`).join('');
...
// ── Helpers de acceso a datos según modo ────────────────────────────
function _calEscGetDatosEditables() {
  if (_calEscEsAdmin()) return _calEsc.adminDatos || _calEscDatosVacios();
  return _calEsc.personalDatos || _calEscDatosVacios();
}

function _calEscSetDatosEditables(datos) {
  if (_calEscEsAdmin()) {
    _calEsc.adminDatos = datos;
  } else {
    _calEsc.personalDatos = datos;
    _calEscGuardarPersonal();
  }
}

function _calEscAsegurarMes(datos, mes) {
  if (!datos.meses) datos.meses = {};
  if (!datos.meses[mes]) datos.meses[mes] = { actividades: [], efemerides: [] };
  if (!datos.meses[mes].actividades) datos.meses[mes].actividades = [];
  if (!datos.meses[mes].efemerides) datos.meses[mes].efemerides = [];
  return datos;
}
...
function _calEscEliminarActividad(mes, idx) {
...
// ── CRUD Efemérides ──────────────────────────────────────────────────
function _calEscAgregarEfemeride(mes) { _calEscModalEfemeride(mes, null, null); }

function _calEscEditarEfemeride(mes, idx) {
  const datos = _calEscGetDatosEditables();
  _calEscAsegurarMes(datos, mes);
  const ef = datos.meses[mes].efemerides[idx];
  if (ef) _calEscModalEfemeride(mes, idx, ef);
}

function _calEscEliminarEfemeride(mes, idx) {
...
function _calEscModalEfemeride(mes, idx, ef) {
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:420px;width:100%;box-shadow:0 8px 32px rgba(0,0,0,0.18);" onclick="event.stopPropagation()">
      ...
      <div style="display:flex;gap:8px;justify-content:flex-end;">
        ...
        <button onclick="_calEscGuardarEfemeride('${mes}',${JSON.stringify(idx)})"
...
function _calEscUsarMiCalendario() {
  if (!confirm('¿Quieres usar tu propio calendario?\nSe copiará el calendario oficial como punto de partida.')) return;
  _calEsc.personalDatos = JSON.parse(JSON.stringify(_calEsc.adminDatos || _calEscDatosVacios()));
  _calEscGuardarPersonal();
  _calEsc.modo = 'personal';
  _calEscRenderizar();
  mostrarToast('Ahora estás usando tu propio calendario', 'success');
}

function _calEscRestaurarAdmin() {
  if (!confirm('¿Restaurar el calendario oficial?\nSe perderán tus cambios personales.')) return;
  _calEsc.personalDatos = null;
  localStorage.removeItem(CAL_ESC_KEY);
  if (window._syncFirebase) _syncFirebase('calendario_escolar', '');
  _calEsc.modo = 'admin';
  _calEscRenderizar();
