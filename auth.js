...
const FIREBASE_STORES = [
  ...
  { store: 'cumpleanos',            key: 'planificadorRA_cumpleanos_v1' },
  ...
  { store: 'drive_folder',         key: 'tinclass_drive_folder_v1' },
  ...
  { store: 'portafolio_foto',       key: 'planificadorRA_portafolio_foto_v1' },
  // portafolio_cv NO va aqui: ahora se guarda en varios documentos ("chunks", ver
  // _guardarPortafolioCV en app.js) porque un CV tipico (3-5MB) no cabe en un blob unico.
];
  ...
  try {
    ...
    const promesas = FIREBASE_STORES.map(async ({ store, key }) => {
      ...
      if (store === 'preferencias') {
        const data = {};
        ['cfg_dark_mode','cfg_fuente_grande','cfg_alertas','cfg_manana','cfg_asistencia_activa','cfg_umbral_riesgo','cfg_umbral_acts','asist_umbral','planificadorRA_touchMode_v1','tinclass_error_alerts_cfg_v1'].forEach(k => {
